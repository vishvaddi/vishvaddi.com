...
export function buildTutorial(t: TutorialTargets): { showTutorialStep: (index: number) => void } {
  ...
  const helpTopics: Array<{ section: string; title: string; text: string }> = [
    ...
    { section: "Sequence", title: "Piano roll", text: "Choose Bass, Lead or Harmony, then drag empty space to draw a note and set its length. Move or resize notes directly; Accent and Slide shape the selected note. Right-click changes velocity." },
