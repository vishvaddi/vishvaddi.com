// MPC performance deck (Create workspace): toolbar, pad banks, the 4×4 pad
...
export function buildPads(): PadsUI {
  ...
  const selectedInputs: Array<{ key: keyof SamplerP; control: KnobControl }> = [];
  function selectedParam(label: string, key: keyof SamplerP, min: number, max: number, step: number): HTMLElement {
    const control = knob({ label, min, max, value: Number(sampleParams[mpc.selectedPad][key]), step, onInput: (value) => { (sampleParams[mpc.selectedPad][key] as number) = value; saveAll(); } });
    selectedInputs.push({ key, control }); return control.el;
  }
  ...
  function paintMpcPads(): void {
    ...
    selectedInputs.forEach(({ key, control }) => control.set(Number(selected[key]), false));
  ...
  function recordTarget(): number { return ctx.isPlaying() ? (clip.play.pads ?? clip.sel) : clip.sel; }
  function recordPadEvent(pad: number, velocity: number): void {
    if (!mpc.recording || !ctx.isPlaying()) return;
    ...
    const playedOffset = started > 0 ? Math.max(-60, Math.min(60, performance.now() - started)) : 0;
  ...
  resampleBtn.addEventListener("click", async () => {
    ...
    try {
      const rendered = await ctx.renderBuffer("pattern");
  ...
  return { mpcPanel, padGrid, padButtons, eventLane, selectedPadLabel, selectedSampleEditor, triggerPerformancePad };
}
