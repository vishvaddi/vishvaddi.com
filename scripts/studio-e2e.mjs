...
try {
  ...
  await page.evaluate(() => { const s = [...document.querySelectorAll('.wa-export')].find((row) => [...row.querySelectorAll('button')].some((b) => (b.textContent || '').includes('Export MIDI'))).querySelector('select'); s.value = 'pattern'; s.dispatchEvent(new Event('change', { bubbles: true })) })
  ...
  await page.screenshot({ path: 'studio-dj-playing.png', fullPage: false })
