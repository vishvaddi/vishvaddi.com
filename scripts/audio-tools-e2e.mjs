...
try {
  ...
  // ── ear training ──
  await page.goto(`${BASE}/audio/ear/#x=note&v=staff&seed=11`, { waitUntil: 'domcontentloaded' })
  await page.evaluate(() => localStorage.removeItem('vv_audio_ear_stats_v1'))
  await page.reload({ waitUntil: 'domcontentloaded' })
  check('ear: six exercise tabs', await page.locator('.ear-tab').count() === 6)
  check('ear: note ID draws a staff with a notehead', await page.locator('.ear-staff .ear-notehead').count() === 1)
  check('ear: seven natural-note choices by default', await page.locator('.ear-choice').count() === 7)
  await page.locator('.ear-choice').first().click()
  const fb = await page.locator('#ear-feedback').textContent()
  check('ear: answering gives feedback and counts the attempt', /Correct|Not quite/.test(fb ?? '') && /1 ·|\/1 ·/.test((await page.locator('#ear-stats').textContent()) ?? ''), `${fb} | ${await page.locator('#ear-stats').textContent()}`)
  await page.locator('.ear-view[data-view="keyboard"]').click()
  check('ear: keyboard view highlights one key', await page.locator('.ear-keys .ear-key.on').count() === 1)
  await page.locator('.ear-view[data-view="fretboard"]').click()
  check('ear: fretboard view marks one position', await page.locator('.ear-fret .ear-dot').count() === 1)
  await page.locator('.ear-tab[data-exercise="chord"]').click()
  await page.locator('.ear-view[data-view="staff"]').click()
  check('ear: chord ID shows three noteheads on the staff by default', await page.locator('.ear-staff .ear-notehead').count() === 3)
  await page.locator('.ear-tab[data-exercise="intervalEar"]').click()
  check('ear: interval ear has a play button and no view toggle', await page.locator('#ear-play').count() === 1 && await page.locator('#ear-views').isHidden())
  await page.click('#ear-play')
  await page.locator('.ear-tab[data-exercise="eq"]').click()
  check('ear: EQ ear offers the seven default bands', await page.locator('.ear-choice').count() === 7)
  await page.click('#ear-play')
  await page.locator('.ear-tab[data-exercise="rhythm"]').click()
  check('ear: rhythm dictation offers four patterns of eight cells', await page.locator('.ear-choice').count() === 4 && await page.locator('.ear-choice').first().locator('.ear-rhythm i').count() === 8)
  await page.click('#ear-play')
  await page.locator('.ear-settings summary').click()
  await page.selectOption('#ear-density', '3')
  check('ear: sixteenth grid setting takes effect', await page.locator('.ear-choice').first().locator('.ear-rhythm i').count() === 16)
  const share = await page.inputValue('#ear-share')
  check('ear: share link carries exercise, view and settings', /#x=rhythm&v=ear&s=/.test(share), share.slice(0, 60))
  await page.reload({ waitUntil: 'domcontentloaded' })
  check('ear: stats survive reload', /\/1 ·/.test((await (async () => { await page.locator('.ear-tab[data-exercise="note"]').click(); return page.locator('#ear-stats').textContent() })()) ?? ''), await page.locator('#ear-stats').textContent())
