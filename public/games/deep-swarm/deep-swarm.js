...
function startAmbient() {
    ...
    const eB = audioCtx.createOscillator();
    const eFilter = audioCtx.createBiquadFilter();
    const eGain = audioCtx.createGain();
    eA.type = 'sawtooth'; eA.frequency.value = 38;
    eB.type = 'sawtooth'; eB.frequency.value = 41;     // detune for thickness
    eFilter.type = 'lowpass'; eFilter.frequency.value = 220;
    eGain.gain.value = 0.018;     // baseline rumble (sub idling)
    eA.connect(eFilter); eB.connect(eFilter);
    eFilter.connect(eGain); eGain.connect(underwaterFilter);
    eA.start(); eB.start();
    ambientPads.push({ engine: true, osc: eA, oscB: eB, gain: eGain, filter: eFilter });
}
...
function updateAmbient() {
    ...
    for (const pad of ambientPads) {
        if (pad.engine) {
            ...
            pad.gain.gain.value = 0.014 + moveT * 0.022 + breath * 0.008;
            // Slight pitch shift with depth (lower = deeper / heavier)
            const baseFreq = 38 - depthPct * 12;
            pad.osc.frequency.value = baseFreq;
...
// Mouse click — fires manual ping during play
...
const NEREID = {
    ...
    corruption: {
        ...
        100: [
            ...
            'Aoshenvel. I understand now. I understand now. I understand now. Pilot — I am sorry. The Coil also loves you. We are not different about this.',
        ],
    },
...
const EVENT_DEFS = [
    ...
    { id: 'distress', minWave: 5, title: 'DISTRESS SIGNAL', text: 'Automated beacon. Previous unit. Signal degraded.',
        choices: [
            ...
            { text: '[2] IGNORE — Continue mission', fn: g => {} },
        ], noChoice: g => {} },
    ...
    { id: 'wreck', minWave: 6, title: 'WRECK DISCOVERY', text: 'Debris field. Previous vessel. Salvage possible.',
        choices: [
            { text: '[1] SALVAGE — Random weapon upgrade', fn: g => { const w = g.player.weapons[Math.floor(Math.random() * g.player.weapons.length)]; if (w && w.level < 8) w.level++; } },
            { text: '[2] STRIP — Gold + materials', fn: g => { g.goldEarned += 50; } },
        ], noChoice: g => {} },
    { id: 'bloom', minWave: 2, title: 'BIOLUMINESCENT BLOOM', text: 'Beautiful. The water glows with living light.',
        choices: [
            ...
            { text: '[2] COLLECT — XP burst', fn: g => { for (let i = 0; i < 15; i++) g.gems.push({ x: g.player.x + (Math.random() - 0.5) * 100, y: g.player.y + (Math.random() - 0.5) * 100, value: 3, size: 4, life: 10 }); } },
        ], noChoice: g => {} },
    { id: 'swarm_intel', minWave: 8, title: 'SWARM INTELLIGENCE', text: 'They\'re adapting. NEREID detects a structural weakness.',
        choices: [
            ...
            { text: '[2] OBSERVE — +25 gold, data logged', fn: g => { g.goldEarned += 25; } },
    ...
    { id: 'lantern_echo', minWave: 15, title: 'LANTERN ECHO', text: 'Probe LANTERN-3 frequencies on the wire. The signal is looping. It is repeating photograph 11.',
        choices: [
            ...
            { text: '[2] CUT THE FREQUENCY — Some photographs should stay sealed', fn: g => { g.goldEarned += 40; } },
    ...
    { id: 'hemobrine_breach', minWave: 12, title: 'HEMOBRINE BREACH', text: 'Hull sensors confirm: the medium at this depth is no longer water. The lamps are illuminating tissue. We are inside something.',
        choices: [
            ...
            { text: '[2] WITHDRAW — The Coil has noticed us', fn: g => { g.player.corruption = Math.max(0, g.player.corruption - 5); } },
...
const LORE_FRAGMENTS = [
    ...
    // === EXPANDED LORE — Pelagos-9 / The Coil / Aoshen ===
    // LAYER 1 — Corporate / Aster Station
    { id: 'c11', layer: 1, text: 'ASTER STATION — Policy Memo\n"All personnel are reminded that \'the Coil\' and \'the Lung\' are not approved terminology. Use \'the Substrate\' in all reports."' },
    ...
    { id: 'p12', layer: 2, text: 'K. Ven — Listening Watch, Day 47\n"The breath is not a wave. It is a phrase. Every 11th hour contains the previous, attenuated. The Coil exhales its memory of every prior exhale."' },
    { id: 'p13', layer: 2, text: 'Signal Theology Pamphlet (anonymous, removed/reposted)\n"Listen long enough and you will hear it. An under-tone. The Coil is calling to something that no longer answers. Possibly killed it. Possibly was killed by it."' },
    ...
    { id: 'd13', layer: 3, text: '[AOSHENVEL — TRANSCRIPTION]\n"When LANTERN-3 took its eleventh photograph, the Coil reached back through the photograph. By the fifteenth, the Coil had begun, in its own way, to wonder."' },
...
function addNereidLog(g, text) {
    ...
    g.nereidLog.unshift({ text, time: g.runTime });
...
const ENEMY_TYPES = {
    ...
    jellyfish: {
        ...
        lore: 'A pressure baroreceptor. The Coil senses you through these membranes. The cells contain no DNA. They never needed any.',
        aberrantTwist: 'jelly_pulse',
    },
    piranha: {
        name: 'Razor Piranha', hp: 8, speed: 90, size: 7, color: '#FF6A4A',
        xp: 1, damage: 3, gold: 1, minWave: 2, ai: 'pack',
        lore: 'A clotting agent. Individually trivial. In swarms they aggregate at wounds. Any wound, including yours.',
    ...
    hermit: {
        ...
        lore: 'A mineral recycler. Charges in straight lines because the Coil prefers efficiency. Soft from behind because the Coil does not yet need rear armour at this depth.',
    },
    puffer: {
        name: 'Bog Puffer', hp: 22, speed: 25, size: 12, color: '#A0D060',
        xp: 2, damage: 4, gold: 2, minWave: 4, ai: 'puff',
        lore: 'An immune burst cell. Detonates on death. The Coil keeps the spikes — they remain useful even without the body around them.',
    },
    ...
    squid: {
        ...
        lore: 'A sensory probe. The Coil sent it to look. It is currently looking back.',
    },
    glowshrimp: {
        ...
        lore: 'A luminescent neuron. Fires bioluminescent darts encoded with Coil signals. The darts remember being shrimp.',
        aberrantTwist: 'shooter_burst',
    },
    eel: {
        name: 'Volt Eel', hp: 30, speed: 100, size: 12, color: '#FFD040', xp: 4, damage: 10, gold: 3, minWave: 8, ai: 'zigzag',
        lore: 'An electrical fiber. It is not chasing you. It is being routed between organs. You happen to be in the way.',
    },
    manta: {
        name: 'Pale Manta', hp: 90, speed: 38, size: 24, color: '#9AB0C8',
        xp: 6, damage: 14, gold: 4, minWave: 9, ai: 'sweep',
        lore: 'A lymphatic sweep tissue. Old. Patient. Has watched fleets vanish for longer than there have been fleets.',
    },
    ...
    anglerfish: {
        ...
        lore: 'A lure-organ. The light is not bioluminescence. It is a NEREID-II status LED frequency, exactly matched. NEREID has chosen not to comment.',
    },
    vampyro: {
        name: 'Vampyroteuthis', hp: 70, speed: 60, size: 15, color: '#A06ACC',
        xp: 5, damage: 11, gold: 4, minWave: 11, ai: 'phase',
        lore: 'Phase tissue. Moves between substrate and visible water. The Coil sends these to places it does not want known.',
    ...
    nightmare: {
        ...
        lore: 'A tooth-deployment node. The Coil grows weapons here. Always smiling because the bone is always ready.',
    },
    ...
    gulper: {
        ...
        lore: 'An intake valve. Lunges with telegraph because the Coil does not yet need to be subtle here. It will be subtle in the Red Layer.',
    ...
    dragonfish: {
        ...
        lore: 'A heat-vent organism. Lives near vents. Mistakes the sub for prey, but mistakes are how the Coil learns.',
    },
    tubeworm: {
        ...
        lore: 'A chemical processor. Cannot move. Spits acid in four directions. Patient. The Coil planted these centuries ago.',
    },
    ...
    voideye: {
        ...
        lore: 'A proprioceptor. The Coil senses its own position through these. It is not looking at you — it is looking through itself.',
    },
    trenchworm: {
        ...
        lore: 'A buried ambush response. Bursts from below without warning. Always close. The Coil keeps emergency responses pre-positioned.',
    },
    ...
    sunfish: {
        ...
        lore: 'A bulk filter. Mostly oblivious. Will kill you with its mass alone. The Coil does not need it to do anything else.',
    },
    polyp: {
        ...
        lore: 'An anchored stinger array. The skin telling you to leave. It does not chase. It does not need to.',
    },
    wolffish: {
        name: 'Pack Wolffish', hp: 26, speed: 80, size: 10, color: '#A06040',
        xp: 2, damage: 6, gold: 2, minWave: 4, ai: 'pack',
        lore: 'A larger clotting agent. The Coil sends these when piranhas are not enough. Bigger. Same purpose.',
    },
    ...
    lanternfish: {
        ...
        lore: 'A navigator beacon. Carries its own lantern. Drifts toward yours. The Coil is curious which way you will turn.',
    },
    glassoct: {
        name: 'Glass Octopus', hp: 50, speed: 65, size: 13, color: '#80E0CC',
        xp: 4, damage: 9, gold: 3, minWave: 7, ai: 'phase',
        lore: 'A camouflage tissue. See-through. Phases out of phase. Cannot be hit while ghosted. Unfair by design.',
    },
    twicrab: {
        name: 'Twilight Crab', hp: 65, speed: 70, size: 12, color: '#7080A0',
        xp: 4, damage: 10, gold: 3, minWave: 8, ai: 'sweep',
        lore: 'A lateral sweep tissue. Sideways always. Pincers wired like wirecutters. The Coil tested these on previous DSV cables.',
    },
    ...
    bonesmoker: {
        ...
        lore: 'A calcium recycler. Spits bone fragments. The bones were inside something else first. The Coil remembers what.',
    },
    whisperer: {
        name: 'Whisperer', hp: 40, speed: 70, size: 11, color: '#C0C0E0',
        xp: 4, damage: 6, gold: 3, minWave: 12, ai: 'curious', isWhisperer: true,
        lore: 'A pressure-wave amplifier. Low threat alone. Speaks in pressure to other organs. They speed up because it asked.',
    },
    ghostray: {
        name: 'Ghost Ray', hp: 130, speed: 45, size: 26, color: '#9080B0',
        xp: 7, damage: 13, gold: 5, minWave: 13, ai: 'phase',
        lore: 'A memory ghost. A manta that learned to disappear, or always did. The records disagree. Both are true.',
    },
    ...
    lurker: {
        ...
        lore: 'A stealth defender. Invisible until close. Then very, very visible. The Coil has been studying ambush.',
    },
    presseel: {
        ...
        lore: 'A pressure-pulse fiber. Fires pressure pulses. The water itself becomes the weapon. The Coil never wastes water.',
    },
    bonecoral: {
        ...
        lore: 'A calcified colony. Fires bone darts. Older than the trench above. The Coil grew them from previous pilots.',
    },
    ...
    listener: {
        ...
        attackCd: 3.0,
        lore: 'An acoustic nerve cluster. Dormant when silent. Wakes when you fire. To the Coil, sonar is a question. To the Listener, your question is heresy.',
    },
    tendrilmass: {
        name: 'Tendril Mass', hp: 200, speed: 35, size: 18, color: '#3A0A1A',
        xp: 10, damage: 18, gold: 8, minWave: 20, ai: 'sweep',
        lore: 'A tentacle defender. Many short mouths. Each has an opinion. The opinions are unanimous.',
    },
    pressureform: {
        name: 'Pressure Form', hp: 150, speed: 80, size: 14, color: '#FF60FF',
        xp: 12, damage: 20, gold: 9, minWave: 22, ai: 'phase',
        lore: 'Pressure given shape. Not biological. Definitely intentional. The Coil chose to make these. We do not know why.',
    },
    ...
    tissuedrift: {
        ...
        lore: 'A drifting capillary cluster. Heals nearby organs by leaking pressure. The Coil patches itself this way.',
    },
    capillaryworm: {
        ...
        lore: 'Bursts from the hemobrine wall. The wall is where it lives. The wall is where it has always lived.',
    },
    hemoclot: {
        name: 'Hemoclot', hp: 14, speed: 110, size: 8, color: '#FF4060',
        xp: 3, damage: 8, gold: 2, minWave: 17, ai: 'pack',
        lore: 'A Red Layer clotting agent. Aggressive. The Coil sends these only when bleeding.',
    },
    ...
    leviathan: {
        ...
        lore: 'Trench-shaping muscle. Three kilometres of contractile tissue. Older than the trench — it dug the trench. The trench is its waste.',
    },
    kraken: {
        name: 'THE WIDOWMAKER', hp: 800, speed: 30, size: 35, color: '#DA6ADA',
        xp: 80, damage: 20, gold: 80, minWave: 10, ai: 'patrol', isBoss: true,
        lore: 'Surface-watch tentacle organ. Stays at 1,500m. Pulls down what surfaces. It does not understand why we return.',
    },
    dreadnought: {
        name: 'OBLIVION-VII', hp: 1200, speed: 45, size: 40, color: '#8A8A8A',
        xp: 120, damage: 30, gold: 120, minWave: 15, ai: 'patrol', isBoss: true,
        lore: 'A class-DSV wreck the Coil reactivated. It still has its old combat protocols. Now an immune-response asset.',
    },
    abyssal_maw: {
        name: 'FATHER MOUTH', hp: 3000, speed: 20, size: 60, color: '#FF4040',
        xp: 200, damage: 40, gold: 200, minWave: 25, ai: 'patrol', isBoss: true,
        lore: 'The wound that does not close. Diameter matches the Pelagos-3 crater. We were not the first to make it. We will not be the last.',
    },
};
...
const WEAPON_DEFS = {
    ...
    // --- EVOLVED WEAPONS (Ball X Pit combinations) ---
...
// --- Weapon evolution recipes (both at max level 8 → fuse) ---
const WEAPON_EVOLUTIONS = [
    // T2 (Evolved) — base + base
...
const UPGRADE_POOL = [
    { id: 'dmg', name: 'DAMAGE +20%', fn: g => { g.player.dmgMult *= 1.2; }, weight: 10 },
    { id: 'speed', name: 'MOVE SPEED +10%', fn: g => { g.player.speed *= 1.1; }, weight: 8 },
    { id: 'maxhp', name: 'MAX HP +25', fn: g => { g.player.maxHp += 25; g.player.hp += 25; }, weight: 8 },
    { id: 'magnet', name: 'MAGNET +30%', fn: g => { g.player.magnetRange *= 1.3; }, weight: 7 },
    ...
    { id: 'xpgain', name: 'XP GAIN +15%', fn: g => { g.player.xpMult *= 1.15; }, weight: 6 },
    { id: 'heal', name: 'HEAL 30 HP', fn: g => { g.player.hp = Math.min(g.player.maxHp, g.player.hp + 30); }, weight: 5 },
    ...
    // Tradeoff upgrades — every powerful effect carries a cost.
    { id: 'overdrive', name: 'OVERDRIVE', desc: '+40% damage. +20% weapon cooldown.', fn: g => { g.player.dmgMult *= 1.4; g.player.cdMult *= 1.20; }, weight: 5 },
    { id: 'combat_stims', name: 'COMBAT STIMS', desc: '+30% move speed. -30 max HP.', fn: g => { g.player.speed *= 1.30; g.player.maxHp = Math.max(20, g.player.maxHp - 30); g.player.hp = Math.min(g.player.hp, g.player.maxHp); }, weight: 5 },
    { id: 'stripped', name: 'CARGO STRIPPED', desc: '+20% move speed. -40% magnet range.', fn: g => { g.player.speed *= 1.20; g.player.magnetRange *= 0.6; }, weight: 5 },
    { id: 'pressure_lens', name: 'PRESSURE LENS', desc: '+40% weapon area. -15% damage.', fn: g => { g.player.areaMult *= 1.40; g.player.dmgMult *= 0.85; }, weight: 5 },
    { id: 'predator_focus', name: 'PREDATOR FOCUS', desc: '+25% damage. -25% weapon area.', fn: g => { g.player.dmgMult *= 1.25; g.player.areaMult *= 0.75; }, weight: 5 },
    { id: 'deep_scavenger', name: 'DEEP SCAVENGER', desc: '+50% XP gain. -30% magnet range.', fn: g => { g.player.xpMult *= 1.5; g.player.magnetRange *= 0.7; }, weight: 5 },
];
...
const INTRO_SCREENS = [
    ...
    { bg: '#020406', text: [
        '[ MOVEMENT BRIEFING ]',
        '',
        '🎮 WASD — Thruster control',
        '   Your sub has inertia. The ocean is heavy.',
        '   You will drift. Plan your movements.',
        '',
        '⚡ SPACE / SHIFT — Emergency dash',
        '   Brief burst of speed. Invulnerable during dash.',
        '   Use it to escape. 0.8s cooldown.',
    ], nereid: '📻 Thrusters nominal. Dash system charged.', delay: 0 },
    { bg: '#020408', text: [
        '[ WEAPONS BRIEFING ]',
        '',
        '📡 Most weapons fire automatically.',
        '   SONAR is MANUAL — press [F] or CLICK to ping.',
        '   Each pulse: damage ring + reveal in the dark.',
        '   Use it deliberately. Auto-fire unlocks later.',
        '',
        '⬆️ Kill creatures → collect XP gems → level up',
        '   Each level: choose 1 of 3 upgrades.',
        '   AUTO-PING SYSTEM unlocks at sonar LV3+.',
    ], nereid: '📻 Weapons hot. Sonar pulse calibrated. Manual fire only — for now.', delay: 0 },
    { bg: '#03050A', text: [
        '[ SANITY ADVISORY ]',
        '',
        '🧠 Your sanity will degrade over time.',
        '   The deeper you go, the faster it falls.',
        '   Below 40%: visual distortion. Phantom contacts.',
        '   Below 10%: NEREID may become... unreliable.',
        '',
        '   Some things down here affect the mind',
        '   in ways we do not fully understand.',
    ], nereid: '📻 Pilot neural baseline recorded. Try to stay calm.', delay: 0 },
    { bg: '#04060C', text: [
        '[ REGARDING THE CREATURES ]',
        '',
        'The organisms you will encounter are catalogued',
        'in your onboard codex. Most are hostile.',
        '',
        'Some exhibit behaviour inconsistent with',
        'known marine biology. Do not attempt',
        'communication. Do not stop moving.',
        '',
        'Specimen retrieval is your PRIMARY objective.',
        'Your survival is... appreciated.',
    ], nereid: '📻 I\'ve read the previous pilot reports. Don\'t read them.', delay: 0 },
    { bg: '#05080E', text: [
        '[ FINAL NOTE ]',
        '',
...
function createGame() {
    ...
    const stake = meta.currentStake || 0;
    ...
    const g = {
        player: {
            ...
            // Death Defiance — Feature 8: Stake 5 removes one
            deathDefiance: stake >= 5 ? 0 : 1,
            // Chain kill tracking
    ...
    // Feature 8: Apply stake modifiers
    if (stake >= 5) { g.player.speed *= 1.5; } // enemies get speed below
    // Add starting weapon
...
function rollCreaturePool() {
    ...
    for (const biome in biomes) {
        ...
        const count = Math.min(list.length, 3 + Math.floor(Math.random() * 3));   // 3–5 per biome
...
function spawnEnemy(g, forceType, forcePos) {
    ...
    // Feature 8: Stake HP modifiers
    let hpMult = waveMult;
    if (stake >= 1) hpMult *= 1.2; // Pressure stake
    ...
    if (!g._seenTypes[type.id]) {
        ...
        const firstLines = {
            jellyfish: 'Contact — pressure baroreceptors. The Coil is sensing us through their membranes. Their cells contain no DNA. They never needed any.',
            piranha: 'Clotting agents. Individually trivial. They swarm wounds. Including ours.',
            squid: 'A sensory probe. The Coil sent it to look at us. Pilot — it is looking back.',
            anglerfish: 'Lure detected. The light is a NEREID-II status LED frequency. Exactly matched. I have chosen not to comment.',
            eel: 'Volt fiber. Not pursuing us. Being routed between organs. We are in the way.',
            leviathan: 'Class designation: THRESHER. Three kilometres of contractile tissue. Pilot — that is the trench, moving.',
            kraken: 'WIDOWMAKER detected. At 1,500m, as recorded. It does not chase. It waits for us to ascend.',
            dreadnought: 'OBLIVION-VII telemetry. The wreck was reactivated. The Coil keeps its dead working.',
            abyssal_maw: 'FATHER MOUTH. Pilot — same dimensions as the Pelagos-3 crater. The wound that does not close.',
        };
...
function firePing(g) {
    ...
    if (g.player._sonarDouble) {
        setTimeout(() => {
            if (game && g.effects) g.effects.push({ type: 'sonar_ring', x: g.player.x, y: g.player.y, radius: 0, maxRadius: area * 0.8, dmg: dmg * 0.7, speed: 320, hit: new Set() });
        }, 280);
    }
    return true;
}
...
function fireWeapons(g, dt) {
    ...
    for (const w of g.player.weapons) {
        ...
        if (w.id === 'torpedo') {
            ...
            let a;
            if (g.player._mouseAim && g._vpCx != null) {
...
function spawnWorldObjects(g, dt) {
    ...
    const types = OBSTACLE_TYPES_BY_ZONE[zone] || OBSTACLE_TYPES_BY_ZONE.SUNLIGHT;
    ...
    const targetCount = zone === 'SUNLIGHT' ? 12 : zone === 'TWILIGHT' ? 16 : zone === 'MIDNIGHT' ? 20 : zone === 'ABYSSAL' ? 24 : zone === 'RED_LAYER' ? 26 : 28;
    ...
    if (g._specialEventCd <= 0 && g.specialEvents.length < 1) {
        ...
        if (kind) {
            ...
            g.specialEvents.push({ kind, x, y, r: kind === 'whalefall' ? 60 : 36, obDepth, life: 60, looted: false, scavengerTimer: 0, ventPulse: 0 });
...
function updateWreckInteraction(g, dt) {
    ...
    if (g.nearestWreck && eHeld) {
        g.salvageHoldTime = (g.salvageHoldTime || 0) + dt;
...
function damageEnemy(g, e, dmg) {
    ...
    // HERMIT — half damage from front (player is in front of hermit if angle from hermit to player matches its facing)
    if (e.typeId === 'hermit') {
        // Front = direction toward player at last frame. Use a simple heuristic: hermit facing = current movement direction.
        // For simplicity, just apply 0.5 damage modifier always — represents tough armoured shell.
        dmg *= 0.5;
    }
    ...
    // Feature 9: Scan bonus +10% damage against scanned type
    if (meta.scannedCreatures.includes(e.typeId)) finalDmg *= 1.1;
...
// --- Level up ---
function checkEvolutions(g) {
    for (const evo of WEAPON_EVOLUTIONS) {
        const wA = g.player.weapons.find(w => w.id === evo.a);
        const wB = g.player.weapons.find(w => w.id === evo.b);
        if (wA && wB && wA.level >= 8 && wB.level >= 8) {
            // FUSE! Remove both, add evolved
            g.player.weapons = g.player.weapons.filter(w => w.id !== evo.a && w.id !== evo.b);
            g.player.weapons.push({ id: evo.result, level: 1, cooldown: 0 });
            g.streak = 'EVOLUTION: ' + evo.name + '!';
            g.streakTimer = 3;
            g.flashTimer = 0.6;
            g.shake = 15;
            g.slowmo = 0.5;
...
function triggerLevelUp(g) {
    ...
    // Check weapon evolutions first
    if (checkEvolutions(g)) { addNereidLog(g, getNereidLine('evolution', g)); return; }
    // Check if can add new weapon
    const ownedWeaponIds = g.player.weapons.map(w => w.id);
    const availableWeapons = Object.keys(WEAPON_DEFS).filter(id => !ownedWeaponIds.includes(id));
...
// =====================================================================
const STAKES = [
    { id: 0, name: 'Normal', color: '#888888', desc: 'Default difficulty' },
    { id: 1, name: 'Pressure', color: '#5AAFDA', desc: 'Enemies +20% HP' },
    { id: 2, name: 'Crushing', color: '#8A4ABA', desc: 'Corruption +50% faster' },
    { id: 3, name: 'Abyssal', color: '#DA4060', desc: 'Aberrant chance 15%' },
    { id: 4, name: 'Hadal', color: '#FF2020', desc: 'Bosses every 5 waves' },
    { id: 5, name: 'Mariana', color: '#FF6040', desc: 'Enemies +50% speed, -1 Defiance' },
];
...
function onTouchStart(e) {
    ...
    for (const t of e.changedTouches) {
        ...
        const dashBtnX = w - 60, dashBtnY = h - 80;
        const dx = t.clientX - dashBtnX, dy = t.clientY - dashBtnY;
        if (Math.sqrt(dx*dx+dy*dy) < 50 && (phase === 'playing' || phase === 'paused')) {
            touchDash.active = true;
            // Trigger dash
            if (phase === 'playing' && game) {
                ...
                if (p.dashCooldown <= 0 && p.dashTimer <= 0) {
                    ...
                    if (touchJoystick.active) {
                        ddx = (touchJoystick.x - touchJoystick.startX);
                        ddy = (touchJoystick.y - touchJoystick.startY);
                    } else ddy = -1;
        ...
        } else if (t.clientX < w / 2 && (phase === 'playing' || phase === 'paused')) {
            touchJoystick = { active: true, startX: t.clientX, startY: t.clientY, x: t.clientX, y: t.clientY, id: t.identifier };
        } else if (phase === 'title' || phase === 'death') {
            // Tap to start
            const num = Math.ceil((t.clientY / h) * 3);
            simulateKey('Enter');
        }
    }
}
...
function update(dt) {
    ...
    if (g.sonarReveal > 0) g.sonarReveal = Math.max(0, g.sonarReveal - dt * 0.5);
    ...
    const thrustForce = p.speed * 4; // acceleration
    ...
    if (g.waveTimer >= 45) {  // longer waves for breathing room (was 30s)
        ...
        // Feature 8: Stake 4 — bosses every 5 waves
        if ((g.stake || 0) >= 4 && g.wave > 5 && g.wave % 5 === 0) {
            const bossTypes = ['kraken','dreadnought','leviathan'];
            spawnBossAt(bossTypes[Math.floor(Math.random() * bossTypes.length)]);
        }
        // Feature 8: Stake 1 unlocked on wave 15
        if (g.wave === 15 && meta.currentStake >= meta.highestStake) {
            const nextStake = meta.highestStake + 1;
            if (nextStake <= 5 && meta.highestStake === meta.currentStake) {
                meta.highestStake = Math.min(5, nextStake);
                saveMeta();
                addNereidLog(g, `NEW STAKE UNLOCKED: ${STAKES[nextStake] ? STAKES[nextStake].name : ''}`);
            }
        }
    }
    ...
    if (g.spawnTimer >= g.spawnRate * ascendMult) {
        g.spawnTimer = 0;
    ...
    for (let i = g.enemies.length - 1; i >= 0; i--) {
        ...
        if (p._facing != null && dToPlayer < 220) {
        ...
        e.speed = (ENEMY_TYPES[e.typeId]?.speed || e.speed) * lightSpeedMult * whisperBoost * (g._enemySpdMult || 1) * (g._feeding ? 1.3 : 1);
        ...
        if (ai === 'drift') {
            ...
            // HERMIT — slow approach, periodic charge with telegraph. Half damage from front, full from behind.
            ...
            if (e._chargeState === 'charge') {
                ...
                if (e._chargeT <= 0) { e._chargeState = null; e._chargeCd = 4 + Math.random() * 2; }
            ...
            if (e._shootCd <= 0 && dToPlayer < (def.attackRange || 350)) {
                ...
                for (let pi = 0; pi < spread; pi++) {
                    const aOff = spread > 1 ? (pi - (spread - 1) / 2) * 0.18 : 0;
                    const a = angleToPlayer + aOff;
                    g.projectiles.push({ x: e.x, y: e.y, vx: Math.cos(a) * sp, vy: Math.sin(a) * sp, dmg, aoe: 14, life: 2.5, pierce: 0, color: col, enemy: true });
                }
        ...
        if (e.hp <= 0) {
            ...
            if (!e.ghost) {
                ...
                g.goldEarned += Math.floor(e.gold * (g._goldMult || 1) * feedReward);
    ...
    for (let i = g.gems.length - 1; i >= 0; i--) {
        ...
        if (d < p.magnetRange) {
            ...
            const pullSpeed = 300 * (1 - d / p.magnetRange);
            gem.x += Math.cos(a) * pullSpeed * dt;
            gem.y += Math.sin(a) * pullSpeed * dt;
        }
        if (d < 15) {
            const xpGain = Math.floor(gem.value * p.xpMult);
            p.xp += xpGain;
            g.gemsCollected++;
            sfxCollect();
    ...
    if (!p._noCorrupt) {
        ...
        if ((g.stake || 0) >= 2) corruptRate *= 1.5;
    ...
    // --- DEPTH SYSTEM ---
    if (!g.ascending) {
        // Descending — non-linear free-fall
        const _t = g.runTime;
        g.depth = Math.floor(_t * 5.0 + Math.pow(_t, 1.4) * 0.05);
        ...
        if (g.depth <= 0 && phase === 'playing') {
            ...
            phase = 'death'; // reuse death screen but mark as surfaced
            g._surfaced = true;
    ...
    if (g.marinSnow.length < 90 && Math.random() < 0.20) {
    ...
    if (g.depth > 1000 && g.runTime - g.ambient.lastCreak > 18 + Math.random() * 14) {
...
function draw() {
    ...
    for (const z of zones) {
        if (g.depth >= z.depth && g._lastZone < z.depth) {
            g.streak = z.name; g.streakTimer = 3;
            g._lastZone = z.depth;
        }
    }
    ...
    for (const ob of (g.obstacles || [])) {
        ...
        if (ob.kind === 'rock' || ob.kind === 'spire' || ob.kind === 'vent' || ob.kind === 'organic' || ob.kind === 'crystal') {
    ...
    // --- DARKNESS OVERLAY (offscreen canvas approach — reliable) ---
    {
        ...
        const darkAlpha = Math.min(0.7, darkDelay * 0.7);
        ...
        if (darkAlpha > 0.03) {
            ...
            const lightR = Math.max(180, 400 * (1 - darkProgress * 0.45));
    ...
    if (isTouchDevice && (phase === 'playing' || phase === 'paused')) {
...
function drawMinimalHUD(w, h, g, pal, vpCx, vpCy, vpR) {
    ...
    // ---- TOP-RIGHT: LOADOUT (active cards + level-up upgrades) ----
    const loadout = [];
    ...
    if (loadout.length) {
        ...
        for (let i = 0; i < loadout.length; i++) {
            ...
            // Icon background — different border for cards vs level-up
            ctx.fillStyle = 'rgba(8,12,18,0.85)';
            ctx.beginPath(); ctx.roundRect(ix, iy, iconSize, iconSize, 3); ctx.fill();
            ctx.strokeStyle = u.src === 'card' ? hexA('#A06ACC', 0.7) : hexA(ic.color, 0.6);
            ctx.lineWidth = 1;
...
function drawSidePanels(w, h, g, pal, vpCx, vpCy, vpR) { /* DEAD CODE — replaced by drawMinimalHUD */ return;
    const railW = g._railW || 240;
    const leftX = 0;
    const rightX = w - railW;
    const panelTop = g._railTopY != null ? g._railTopY : 0;
    const panelBot = g._railBotY != null ? g._railBotY : h;
    const panelH = panelBot - panelTop;
    const p = g.player;
    const sanity = 100 - (p.corruption || 0);

    // Backgrounds (narrower, screen-edge anchored)
    drawPanelBg(leftX, panelTop, railW, panelH, pal);
    drawPanelBg(rightX, panelTop, railW, panelH, pal);

    // RIM ARC BARS — HULL on the left half, MIND on the right half. Same radius, same sweep, mirror-symmetric.
    drawRimBar(g, vpCx, vpCy, vpR, 'left',  'HULL', p.hp / p.maxHp, p.hp > p.maxHp * 0.5 ? '#4A9A6A' : p.hp > p.maxHp * 0.25 ? '#DAA520' : '#DA4060', `${Math.floor(p.hp)}/${p.maxHp}`, pal);
    drawRimBar(g, vpCx, vpCy, vpR, 'right', 'MIND', sanity / 100,   sanity > 60 ? '#7AAACC' : sanity > 30 ? '#A06ACC' : '#DA4060', `${Math.floor(sanity)}%`, pal);

    // ============================================================
    // LEFT PANEL — SCORE · VITALS · ABILITIES · WEAPONS
    // ============================================================
    const lx = leftX + 14;
    const lw = railW - 28;
    let ly = panelTop + 14;

    // SCORE — big number at top
    const flash = Math.min(1, g.scoreFlash || 0);
    const scoreColor = flash > 0.05 ? '#FFFFA0' : '#FFE082';
    ctx.textAlign = 'left'; ctx.font = 'bold 11px monospace';
    ctx.fillStyle = pal.textDim;
    ctx.fillText('SCORE', lx, ly);
    ly += 16;
    ctx.fillStyle = scoreColor; ctx.font = `bold ${24 + flash * 4}px monospace`;
    ctx.fillText((g.score || 0).toLocaleString(), lx, ly + 8);
    ly += 28;
    if (meta.bestScore) {
        ctx.fillStyle = '#7A6A40'; ctx.font = '10px monospace';
        ctx.fillText(`BEST  ${meta.bestScore.toLocaleString()}`, lx, ly);
        ly += 14;
    }
    ly += 8;
    drawPanelDivider(lx, ly, lw, pal); ly += 12;

    // VITALS
    const hpPct = p.hp / p.maxHp;
    const hpColor = hpPct > 0.5 ? '#4A9A6A' : hpPct > 0.25 ? '#DAA520' : '#DA4060';
    drawPanelBar(lx, ly, lw, 'HULL', `${Math.floor(p.hp)}/${p.maxHp}`, hpPct, hpColor, pal);
    ly += 26;
    const sanColor = sanity > 60 ? '#7AAACC' : sanity > 30 ? '#A06ACC' : '#DA4060';
    drawPanelBar(lx, ly, lw, 'MIND', `${Math.floor(sanity)}%`, sanity / 100, sanColor, pal);
    ly += 26;
    ly += 6;
    drawPanelDivider(lx, ly, lw, pal); ly += 12;

    // ABILITIES — DASH + PING (key cap + status)
    const dashReady = p.dashCooldown <= 0;
    drawAbilityRow(lx, ly, lw, 'SPACE', 'THRUST', dashReady ? 'READY' : `${p.dashCooldown.toFixed(1)}s`, dashReady, pal.accent, pal);
    ly += 28;
    if (p._sonarManual && !p._sonarAuto) {
        const sw = p.weapons.find(w2 => w2.id === 'sonar');
        if (sw) {
            const pingReady = sw.cooldown <= 0;
            drawAbilityRow(lx, ly, lw, 'F', 'PING', pingReady ? 'READY' : `${sw.cooldown.toFixed(1)}s`, pingReady, '#80FFE0', pal);
            ly += 28;
        }
    }
    ly += 6;
    ...
    // WEAPONS — vertical stack, each row: cooldown ring + code + level pips
    ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
    ctx.fillStyle = hexA(pal.accent, 0.6);
    ctx.fillText('ARMORY', lx, ly); ly += 12;
    const wepLabels = { sonar: 'SONAR', torpedo: 'TORPEDO', field: 'FIELD', depthcharge: 'D.CHARGE', harpoon: 'HARPOON', lure: 'LURE', tsunami: 'TSUNAMI', leviathan_lance: 'LANCE', abyssal_mine: 'MINEFIELD', maelstrom: 'MAELSTROM', sirens_call: 'SIREN', wrath: 'WRATH' };
    for (const ww of p.weapons) {
        const def = WEAPON_DEFS[ww.id];
        const cdMax = def.baseCooldown * p.cdMult;
        const cdPct = Math.max(0, Math.min(1, ww.cooldown / cdMax));
        const ready = cdPct <= 0;
        const rowH = 22;
        // Background
        ctx.fillStyle = ready ? hexA(pal.accent, 0.10 + Math.sin(g.runTime * 3) * 0.04) : 'rgba(8,16,24,0.5)';
        ctx.beginPath(); ctx.roundRect(lx, ly, lw, rowH, 3); ctx.fill();
        // Ready glow border
        ctx.strokeStyle = ready ? hexA(pal.accent, 0.55) : '#0C1B2A'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(lx, ly, lw, rowH, 3); ctx.stroke();
        // Cooldown ring
        const rcX = lx + 11, rcY = ly + rowH / 2;
        ctx.strokeStyle = '#091420'; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.arc(rcX, rcY, 8, 0, PI2); ctx.stroke();
        ctx.strokeStyle = ready ? pal.accent : hexA(pal.accent, 0.6);
        ctx.beginPath(); ctx.arc(rcX, rcY, 8, -Math.PI / 2, -Math.PI / 2 + (1 - cdPct) * PI2); ctx.stroke();
        // Name
        ctx.fillStyle = ready ? pal.text : '#3A5A50'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        ctx.fillText(wepLabels[ww.id] || ww.id.toUpperCase(), lx + 24, ly + rowH / 2 + 3);
        // Level pips
        const pipsX = lx + lw - 60;
        for (let pi = 0; pi < 8; pi++) {
            const filled = pi < ww.level;
            ctx.fillStyle = filled ? (def.evolved ? '#FFD040' : pal.accent) : '#1A3040';
            ctx.fillRect(pipsX + pi * 6, ly + rowH / 2 - 1, 4, 2);
        }
        // Evolved star
        if (def.evolved) { ctx.fillStyle = '#FFD040'; ctx.font = 'bold 10px monospace'; ctx.fillText('★', lx + lw - 8, ly + 9); }
        ly += rowH + 3;
    }

    // ============================================================
    // RIGHT PANEL — TIME · KILLS · OBJECTIVES · COMBO · XP/LEVEL
    // ============================================================
    const rx = rightX + 14;
    const rw = railW - 28;
    let ry = panelTop + 14;

    // STATS — TIME, KILLS, GOLD
    const mins = Math.floor(g.runTime / 60), secs = Math.floor(g.runTime % 60);
    ctx.textAlign = 'left'; ctx.font = 'bold 11px monospace';
    ctx.fillStyle = pal.textDim; ctx.fillText('TIME', rx, ry);
    ctx.textAlign = 'right'; ctx.fillStyle = pal.text; ctx.font = 'bold 14px monospace';
    ctx.fillText(`${mins.toString().padStart(2,'0')}:${secs.toString().padStart(2,'0')}`, rx + rw, ry);
    ry += 18;
    ctx.textAlign = 'left'; ctx.font = 'bold 11px monospace';
    ctx.fillStyle = pal.textDim; ctx.fillText('KILLS', rx, ry);
    ctx.textAlign = 'right'; ctx.fillStyle = '#FFD040'; ctx.font = 'bold 12px monospace';
    ctx.fillText(`${g.kills}`, rx + rw, ry);
    ry += 16;
    ctx.textAlign = 'left'; ctx.font = 'bold 11px monospace';
    ctx.fillStyle = pal.textDim; ctx.fillText('GOLD', rx, ry);
    ctx.textAlign = 'right'; ctx.fillStyle = '#DAA520'; ctx.font = 'bold 12px monospace';
    ctx.fillText(`${g.goldEarned}g`, rx + rw, ry);
    ry += 18;
    drawPanelDivider(rx, ry, rw, pal); ry += 12;

    // OBJECTIVES — DIVE BRIEF
    if (g.objectives && g.objectives.length) {
        ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        ctx.fillStyle = hexA(pal.accent, 0.7);
        ctx.fillText('DIVE BRIEF', rx, ry); ry += 12;
        for (const o of g.objectives) {
            const oh = 32;
            ctx.fillStyle = o.complete ? 'rgba(40,80,40,0.55)' : 'rgba(8,16,24,0.7)';
            ctx.beginPath(); ctx.roundRect(rx, ry, rw, oh, 3); ctx.fill();
            ctx.strokeStyle = o.complete ? '#80E0A0' : hexA(pal.accent, 0.35); ctx.lineWidth = 1;
            ctx.beginPath(); ctx.roundRect(rx, ry, rw, oh, 3); ctx.stroke();
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.beginPath(); ctx.arc(rx + 8, ry + 10, 3, 0, PI2); ctx.fill();
            ctx.fillStyle = o.complete ? '#A0E8B0' : pal.text; ctx.font = '11px monospace';
            const briefMax = Math.floor((rw - 30) / 5.5);
            const brief = o.brief.length > briefMax ? o.brief.slice(0, briefMax - 1) + '…' : o.brief;
            ctx.fillText(brief, rx + 18, ry + 13);
            const pct = Math.max(0, Math.min(1, o.progress / o.target));
            ctx.fillStyle = 'rgba(0,0,0,0.5)';
            ctx.fillRect(rx + 18, ry + 18, rw - 30, 3);
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.fillRect(rx + 18, ry + 18, (rw - 30) * pct, 3);
            ctx.fillStyle = pal.textDim; ctx.font = '10px monospace'; ctx.textAlign = 'right';
            const pTxt = (o.type === 'depth') ? `${Math.floor(o.progress)}/${o.target}m`
                       : (o.type === 'time' || o.type === 'no_hit') ? `${Math.floor(o.progress)}/${o.target}s`
                       : `${Math.floor(o.progress)}/${o.target}`;
            ctx.fillText(pTxt, rx + rw - 6, ry + 28);
            ctx.textAlign = 'left';
            ry += oh + 4;
        }
        ry += 6;
    ...
    // COMBO — SIGNAL × YIELD + CHAIN
    const sc = g.scoreCombo || {};
    const ev = sc.lastEvent;
    const recent = ev && (g.runTime - ev.time < 2.5);
    const chipW = (rw - 18) / 2;
    // SIGNAL
    ctx.fillStyle = '#0A1422';
    ctx.beginPath(); ctx.roundRect(rx, ry, chipW, 28, 4); ctx.fill();
    ctx.strokeStyle = hexA('#80B0FF', 0.5); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.roundRect(rx, ry, chipW, 28, 4); ctx.stroke();
    ctx.fillStyle = '#3A5060'; ctx.font = '9px monospace'; ctx.textAlign = 'center';
    ctx.fillText('SIGNAL', rx + chipW / 2, ry + 9);
    ctx.fillStyle = '#80B0FF'; ctx.font = 'bold 13px monospace';
    ctx.fillText(recent ? `${ev.chips}` : '—', rx + chipW / 2, ry + 22);
    // ×
    ctx.fillStyle = pal.text; ctx.font = 'bold 12px monospace';
    ctx.fillText('×', rx + rw / 2, ry + 18);
    // YIELD
    const yX = rx + rw - chipW;
    ctx.fillStyle = '#1A0A0A';
    ctx.beginPath(); ctx.roundRect(yX, ry, chipW, 28, 4); ctx.fill();
    const multHi = recent && ev.mult >= 3;
    ctx.strokeStyle = multHi ? '#FFD040' : hexA('#DA6040', 0.5); ctx.lineWidth = multHi ? 1.4 : 1;
    ctx.beginPath(); ctx.roundRect(yX, ry, chipW, 28, 4); ctx.stroke();
    ctx.fillStyle = '#5A2A20'; ctx.font = '9px monospace';
    ctx.fillText('YIELD', yX + chipW / 2, ry + 9);
    ctx.fillStyle = multHi ? '#FFD040' : '#DA6040'; ctx.font = 'bold 13px monospace';
    ctx.fillText(recent ? `${ev.mult.toFixed(1)}×` : '—', yX + chipW / 2, ry + 22);
    ry += 36;

    // CHAIN line
    const chain = sc.chainCount || 0;
    if (chain > 1) {
        ctx.fillStyle = pal.textDim; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        ctx.fillText('CHAIN', rx, ry);
        ctx.fillStyle = chain >= 10 ? '#FFD040' : pal.accent; ctx.font = 'bold 14px monospace'; ctx.textAlign = 'right';
        ctx.fillText(`×${chain}`, rx + rw, ry);
        ry += 8;
        // timer bar
        const ctPct = Math.min(1, (sc.chainTimer || 0) / 1.2);
        ctx.fillStyle = 'rgba(0,0,0,0.5)'; ctx.fillRect(rx, ry, rw, 2);
        ctx.fillStyle = hexA(pal.accent, 0.8); ctx.fillRect(rx, ry, rw * ctPct, 2);
        ry += 10;
    }

    // === Bottom of right panel: XP bar + LEVEL ===
    const xpY = panelBot - 36;
    ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
    ctx.fillStyle = pal.textDim; ctx.fillText('PILOT LEVEL', rx, xpY - 16);
    ctx.fillStyle = '#A0DDD0'; ctx.font = 'bold 22px monospace';
    ctx.fillText(`${p.level}`, rx, xpY + 4);
    const xpNeeded = xpForLevel(p.level);
    const xpPct = Math.max(0, Math.min(1, p.xp / xpNeeded));
    const xpX2 = rx + 40, xpW2 = rw - 40, xpH2 = 10;
    ctx.fillStyle = '#040A14';
    ctx.beginPath(); ctx.roundRect(xpX2, xpY - 6, xpW2, xpH2, 4); ctx.fill();
    if (xpPct > 0) {
        ctx.fillStyle = pal.accent;
        ctx.beginPath(); ctx.roundRect(xpX2 + 1, xpY - 5, (xpW2 - 2) * xpPct, xpH2 - 2, 3); ctx.fill();
    }
    ctx.fillStyle = pal.textDim; ctx.font = '10px monospace'; ctx.textAlign = 'right';
    ctx.fillText(`${p.xp}/${xpNeeded}`, rx + rw, xpY + 12);
    // Defiance
    if (p.deathDefiance > 0) {
        ctx.fillStyle = '#FFD040'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        ctx.fillText(`◆ DEFIANCE ×${p.deathDefiance}`, rx, panelBot - 12);
    }

    // === Bottom of left panel: THREAT ===
    let nearby = 0;
    for (const e of g.enemies) {
        if (e.ghost) continue;
        const dx = e.x - p.x, dy = e.y - p.y;
        if (dx * dx + dy * dy < 600 * 600) nearby++;
    }
    ctx.fillStyle = pal.textDim; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
    ctx.fillText('THREAT', lx, panelBot - 28);
    const threatColor = nearby > 30 ? '#DA4060' : nearby > 15 ? '#DAA520' : pal.accent;
    ctx.fillStyle = threatColor; ctx.font = 'bold 18px monospace'; ctx.textAlign = 'right';
    ctx.fillText(`${nearby}`, lx + lw, panelBot - 14);
}
...
// =====================================================================
function drawDepthArc(g, pal, vpCx, vpCy, vpR) { /* DEAD CODE — replaced by drawRimBar HULL/MIND */ return;
    const trackR = vpR + 16;
    const startA = -Math.PI / 2 - 0.05;       // just before 12 o'clock
    const endA   =  Math.PI / 2 + 0.05;       // just past 6 o'clock
    const sweep  = endA - startA;             // ~PI (180°)
    const depthPct = Math.min(1, g.depth / 6000);
    // Track
    ctx.strokeStyle = 'rgba(8,12,20,0.85)';
    ctx.lineWidth = 6;
    ctx.beginPath(); ctx.arc(vpCx, vpCy, trackR, startA, endA, true); ctx.stroke();
    // Fill — sweeps counter-clockwise (top → left → bottom) as we descend
    const fillEnd = startA - depthPct * sweep;
    const danger = depthPct > 0.7;
    const fillCol = danger ? '#DA4060' : depthPct > 0.4 ? '#A06ACC' : pal.accent;
    ctx.strokeStyle = fillCol; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.arc(vpCx, vpCy, trackR, startA, fillEnd, true); ctx.stroke();
    // Glow behind fill
    ctx.strokeStyle = hexA(fillCol, 0.18); ctx.lineWidth = 12;
    ctx.beginPath(); ctx.arc(vpCx, vpCy, trackR, startA, fillEnd, true); ctx.stroke();
    // Milestone ticks (zone transitions). Labels far enough out to never collide with porthole.
    const milestones = [
        { d: 200,  l: 'TWI', c: '#40CCAA' },
        { d: 1000, l: 'MID', c: '#4A6ADA' },
        { d: 2000, l: 'ABY', c: '#8A4ABA' },
        { d: 3800, l: 'TIT', c: '#C0AA60' },
        { d: 4000, l: 'HAD', c: '#DA3050' },
    ];
    ctx.font = 'bold 10px monospace';
    for (const m of milestones) {
        const mPct = m.d / 6000;
        const a = startA - mPct * sweep;
        const reached = g.depth >= m.d;
        ctx.strokeStyle = reached ? m.c : 'rgba(80,100,120,0.6)';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(vpCx + Math.cos(a) * (trackR - 5), vpCy + Math.sin(a) * (trackR - 5));
        ctx.lineTo(vpCx + Math.cos(a) * (trackR + 8), vpCy + Math.sin(a) * (trackR + 8));
        ctx.stroke();
        const lx = vpCx + Math.cos(a) * (trackR + 18);
        const ly = vpCy + Math.sin(a) * (trackR + 18) + 3;
        ctx.fillStyle = reached ? hexA(m.c, 0.95) : 'rgba(120,140,160,0.55)';
        ctx.textAlign = 'right'; // all labels are on the left half — right-aligned to the porthole
        ctx.fillText(m.l, lx, ly);
    }
    // Current depth marker dot
    const headA = startA - depthPct * sweep;
    ctx.fillStyle = fillCol;
    ctx.beginPath(); ctx.arc(vpCx + Math.cos(headA) * trackR, vpCy + Math.sin(headA) * trackR, 4, 0, PI2); ctx.fill();
    drawGlow(ctx, fillCol, vpCx + Math.cos(headA) * trackR, vpCy + Math.sin(headA) * trackR, 12, 0.7);
}
...
// =====================================================================
function drawCornerPanels(g, pal, w, h, vpCx, vpCy, vpR) { /* DEAD CODE */ return;
    const p = g.player;
    const sanity = 100 - (p.corruption || 0);
    const padX = 14;
    const bottomPad = 110 + 16;     // above bottom HUD
    // ---- BOTTOM-LEFT: vital bars (HP / MIND) ----
    const blW = 180, blH = 56;
    const blX = padX, blY = h - bottomPad - blH;
    ctx.fillStyle = 'rgba(2,6,10,0.78)';
    ctx.beginPath(); ctx.roundRect(blX, blY, blW, blH, 4); ctx.fill();
    ctx.strokeStyle = hexA(pal.accentDim, 0.4); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.roundRect(blX, blY, blW, blH, 4); ctx.stroke();
    // HP bar
    const hpPct = p.hp / p.maxHp;
    const hpColor = hpPct > 0.5 ? '#4A9A6A' : hpPct > 0.25 ? '#DAA520' : '#DA4060';
    drawCornerBar(blX + 10, blY + 10, blW - 20, 'HULL', `${Math.floor(p.hp)}/${p.maxHp}`, hpPct, hpColor, pal);
    const sanColor = sanity > 60 ? '#7AAACC' : sanity > 30 ? '#A06ACC' : '#DA4060';
    drawCornerBar(blX + 10, blY + 32, blW - 20, 'MIND', `${Math.floor(sanity)}%`, sanity / 100, sanColor, pal);

    // ---- BOTTOM-RIGHT: objectives (compact stack) ----
    if (g.objectives && g.objectives.length) {
        const brW = 240, brH = 12 + g.objectives.length * 18 + 6;
        const brX = w - padX - brW, brY = h - bottomPad - brH;
        ctx.fillStyle = 'rgba(2,6,10,0.78)';
        ctx.beginPath(); ctx.roundRect(brX, brY, brW, brH, 4); ctx.fill();
        ctx.strokeStyle = hexA(pal.accent, 0.35); ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(brX, brY, brW, brH, 4); ctx.stroke();
        ctx.fillStyle = hexA(pal.accent, 0.7); ctx.font = 'bold 10px monospace'; ctx.textAlign = 'left';
        ctx.fillText('DIVE BRIEF', brX + 8, brY + 10);
        let oy = brY + 22;
        for (const o of g.objectives) {
            const pct = Math.max(0, Math.min(1, o.progress / o.target));
            // Status dot
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.beginPath(); ctx.arc(brX + 12, oy + 4, 3, 0, PI2); ctx.fill();
            // Brief
            ctx.fillStyle = o.complete ? '#A0E8B0' : pal.text;
            ctx.font = '11px monospace'; ctx.textAlign = 'left';
            const briefMaxChars = 28;
            const brief = o.brief.length > briefMaxChars ? o.brief.slice(0, briefMaxChars - 1) + '…' : o.brief;
            ctx.fillText(brief, brX + 22, oy + 7);
            // Progress bar (right edge)
            const pbX = brX + 170, pbW = 60;
            ctx.fillStyle = 'rgba(0,0,0,0.5)';
            ctx.fillRect(pbX, oy + 3, pbW, 4);
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.fillRect(pbX, oy + 3, pbW * pct, 4);
            ctx.fillStyle = pal.textDim; ctx.font = '10px monospace'; ctx.textAlign = 'right';
            ctx.fillText(`${Math.floor(o.progress)}/${o.target}`, brX + brW - 8, oy + 7);
            oy += 18;
        }
    }

    // ---- TOP-RIGHT: SIGNAL × YIELD (only when there's a recent hit) ----
    const sc = g.scoreCombo || {};
    const ev = sc.lastEvent;
    if (ev && (g.runTime - ev.time < 2.5)) {
        const tw = 180, th = 32;
        const tx = w - padX - tw, ty = 56 + 8;
        ctx.fillStyle = 'rgba(2,6,10,0.78)';
        ctx.beginPath(); ctx.roundRect(tx, ty, tw, th, 4); ctx.fill();
        ctx.strokeStyle = hexA('#FFD040', 0.45); ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(tx, ty, tw, th, 4); ctx.stroke();
        ctx.font = 'bold 10px monospace'; ctx.textAlign = 'center';
        ctx.fillStyle = '#80B0FF'; ctx.fillText(`${ev.chips}`, tx + 36, ty + 21);
        ctx.fillStyle = '#3A5060'; ctx.font = '9px monospace';
        ctx.fillText('SIGNAL', tx + 36, ty + 11);
        ctx.fillStyle = pal.text; ctx.font = 'bold 12px monospace';
        ctx.fillText('×', tx + tw / 2, ty + 22);
        ctx.fillStyle = ev.mult >= 3 ? '#FFD040' : '#DA6040'; ctx.font = 'bold 10px monospace';
        ctx.fillText(`${ev.mult.toFixed(1)}×`, tx + tw - 36, ty + 21);
        ctx.fillStyle = '#5A2A20'; ctx.font = '9px monospace';
        ctx.fillText('YIELD', tx + tw - 36, ty + 11);
    }
}
...
// =====================================================================
function drawPortholeRimArcs(g, pal, vpCx, vpCy, vpR) { /* DEAD CODE — replaced by drawRimBar */ return;
    // Sit just outside the porthole bezel (which is 8px wide at vpR+4)
    const trackR = vpR + 14;
    const fillR  = vpR + 14;
    const tickR  = vpR + 22;

    // ---- LEFT ARC: DEPTH METER (from -135° at top-left → +135° at bottom-left, sweeping CCW through 180°) ----
    {
        const startA = -Math.PI * 0.78;   // ~140° (upper-left)
        const endA   =  Math.PI * 0.78;   // ~140° (lower-left), through PI (left)
        // The fill direction: we want it to "fill" as we descend
        const depthPct = Math.min(1, g.depth / 6000);
        // Track (background)
        ctx.strokeStyle = 'rgba(8,12,20,0.9)';
        ctx.lineWidth = 8;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, trackR, startA, endA, true); ctx.stroke();
        // Filled portion (length proportional to depth)
        const sweep = (endA - startA + PI2) % PI2; // total arc length in radians
        // start at top, fill counter-clockwise toward bottom
        const fillEnd = startA - depthPct * sweep;
        const danger = depthPct > 0.7;
        const fillCol = danger ? '#DA4060' : depthPct > 0.4 ? '#A06ACC' : pal.accent;
        ctx.strokeStyle = fillCol;
        ctx.lineWidth = 6;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, fillR, startA, fillEnd, true); ctx.stroke();
        // Soft glow outside the fill
        ctx.strokeStyle = hexA(fillCol, 0.18);
        ctx.lineWidth = 14;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, fillR, startA, fillEnd, true); ctx.stroke();
        // Zone milestone tick marks: 200, 1000, 2000, 3800, 4000m
        const milestones = [
            { d: 200,  l: 'TWI', c: '#40CCAA' },
            { d: 1000, l: 'MID', c: '#4A6ADA' },
            { d: 2000, l: 'ABY', c: '#8A4ABA' },
            { d: 3800, l: 'TITANIC', c: '#C0AA60' },
            { d: 4000, l: 'HAD', c: '#DA3050' },
        ];
        ctx.font = 'bold 10px monospace';
        for (const m of milestones) {
            const mPct = m.d / 6000;
            const mAng = startA - mPct * sweep;
            const reached = g.depth >= m.d;
            ctx.strokeStyle = reached ? m.c : 'rgba(80,100,120,0.55)';
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(vpCx + Math.cos(mAng) * (vpR + 2), vpCy + Math.sin(mAng) * (vpR + 2));
            ctx.lineTo(vpCx + Math.cos(mAng) * (vpR + 14), vpCy + Math.sin(mAng) * (vpR + 14));
            ctx.stroke();
            // Label outside
            const lx = vpCx + Math.cos(mAng) * (tickR + 4);
            const ly = vpCy + Math.sin(mAng) * (tickR + 4);
            ctx.fillStyle = reached ? hexA(m.c, 0.95) : 'rgba(120,140,160,0.5)';
            ctx.textAlign = Math.cos(mAng) > 0 ? 'left' : 'right';
            ctx.fillText(m.l, lx + (Math.cos(mAng) > 0 ? 2 : -2), ly + 3);
        }
        // Current-depth marker (a bright dot riding the arc)
        const headA = startA - depthPct * sweep;
        ctx.fillStyle = fillCol;
        ctx.beginPath(); ctx.arc(vpCx + Math.cos(headA) * fillR, vpCy + Math.sin(headA) * fillR, 4, 0, PI2); ctx.fill();
        // Bright pulse around it
        drawGlow(ctx, fillCol, vpCx + Math.cos(headA) * fillR, vpCy + Math.sin(headA) * fillR, 14, 0.7);
    }

    // ---- RIGHT ARC: YIELD METER (mirror — top-right → bottom-right, fills with current yield) ----
    {
        const startA = -Math.PI * 0.22;   // upper-right
        const endA   =  Math.PI * 0.22;   // lower-right (sweep CW through 0)
        const sweep  = endA - startA;     // ~80° on the right side
        const sc = g.scoreCombo || {};
        const ev = sc.lastEvent;
        const recent = ev && (g.runTime - ev.time < 2.5);
        const liveMult = recent ? Math.min(8, ev.mult) : 1;
        const yieldPct = Math.min(1, (liveMult - 1) / 7); // 1× empty, 8× full
        // Track
        ctx.strokeStyle = 'rgba(8,12,20,0.9)';
        ctx.lineWidth = 8;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, trackR, startA, endA); ctx.stroke();
        // Fill (top → bottom proportional to yield)
        const fillEnd = startA + yieldPct * sweep;
        const yColor = liveMult >= 5 ? '#FFD040' : liveMult >= 3 ? '#FF8040' : '#DA6040';
        ctx.strokeStyle = yColor; ctx.lineWidth = 6;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, fillR, startA, fillEnd); ctx.stroke();
        // Glow
        ctx.strokeStyle = hexA(yColor, 0.22); ctx.lineWidth = 14;
        ctx.beginPath(); ctx.arc(vpCx, vpCy, fillR, startA, fillEnd); ctx.stroke();
        // Tick marks at 2×, 3×, 5×, 8×
        const yticks = [2, 3, 5, 8];
        ctx.font = 'bold 10px monospace';
        for (const yt of yticks) {
            const yp = (yt - 1) / 7;
            const ya = startA + yp * sweep;
            const hit = liveMult >= yt;
            ctx.strokeStyle = hit ? yColor : 'rgba(80,100,120,0.55)';
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(vpCx + Math.cos(ya) * (vpR + 2), vpCy + Math.sin(ya) * (vpR + 2));
            ctx.lineTo(vpCx + Math.cos(ya) * (vpR + 14), vpCy + Math.sin(ya) * (vpR + 14));
            ctx.stroke();
            ctx.fillStyle = hit ? yColor : 'rgba(120,140,160,0.5)';
            ctx.textAlign = 'left';
            ctx.fillText(`${yt}×`, vpCx + Math.cos(ya) * (tickR + 4) + 2, vpCy + Math.sin(ya) * (tickR + 4) + 3);
        }
        // Live yield head dot
        if (yieldPct > 0) {
            const headA2 = startA + yieldPct * sweep;
            ctx.fillStyle = yColor;
            ctx.beginPath(); ctx.arc(vpCx + Math.cos(headA2) * fillR, vpCy + Math.sin(headA2) * fillR, 4, 0, PI2); ctx.fill();
            drawGlow(ctx, yColor, vpCx + Math.cos(headA2) * fillR, vpCy + Math.sin(headA2) * fillR, 14, 0.7);
        }
    }
}

function drawSideRails(w, h, g, pal, vpCx, vpCy, vpR) { /* DEAD CODE — replaced by drawSidePanels */ return;
    const railW = g._railW || 180;
    const topY = g._railTopY != null ? g._railTopY : 72;
    const botY = g._railBotY != null ? g._railBotY : h - 116;
    const leftX = g._railLeftX != null ? g._railLeftX : 0;
    const rightX = g._railRightX != null ? g._railRightX : w - railW;
    const railH = botY - topY;
    const p = g.player;

    // ----- LEFT RAIL: HULL TELEMETRY -----
    drawRailFrame(leftX, topY, railW, railH, pal, 'HULL TELEMETRY');

    const innerPad = 14;
    const innerW = railW - innerPad * 2;
    // Pressure read-out — flat block; the curved DEPTH METER lives on the porthole rim now
    let ly = topY + 26;
    const depthPct2 = Math.min(1, g.depth / 6000);
    const needleColor = depthPct2 > 0.7 ? '#DA4060' : depthPct2 > 0.4 ? '#A06ACC' : pal.accent;
    const atm = (1 + g.depth / 10).toFixed(0);
    ctx.fillStyle = '#040A10';
    ctx.beginPath(); ctx.roundRect(leftX + innerPad, ly, innerW, 46, 5); ctx.fill();
    ctx.strokeStyle = hexA(needleColor, 0.5); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.roundRect(leftX + innerPad, ly, innerW, 46, 5); ctx.stroke();
    ctx.fillStyle = pal.textDim; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
    ctx.fillText('PRESSURE', leftX + railW / 2, ly + 12);
    ctx.fillStyle = needleColor; ctx.font = 'bold 22px monospace';
    ctx.fillText(`${atm}`, leftX + railW / 2 - 14, ly + 34);
    ctx.fillStyle = pal.textDim; ctx.font = 'bold 11px monospace';
    ctx.fillText('ATM', leftX + railW / 2 + 22, ly + 34);
    ly += 56;
    // Status bars — taller and properly spaced
    const temp = Math.max(2, 22 - g.depth / 250);
    drawMiniBar(leftX + innerPad, ly, innerW, 'TEMP', `${temp.toFixed(1)}°C`, 1 - Math.min(1, (22 - temp) / 20), '#5AAFDA', pal);
    ly += 26;
    const hpPct = p.hp / p.maxHp;
    const hpColor = hpPct > 0.5 ? '#4A9A6A' : hpPct > 0.25 ? '#DAA520' : '#DA4060';
    drawMiniBar(leftX + innerPad, ly, innerW, 'HULL', `${Math.floor(p.hp)}/${p.maxHp}`, hpPct, hpColor, pal);
    ly += 26;
    const sanity = 100 - (p.corruption || 0);
    const sanColor = sanity > 60 ? '#7AAACC' : sanity > 30 ? '#A06ACC' : '#DA4060';
    drawMiniBar(leftX + innerPad, ly, innerW, 'MIND', `${Math.floor(sanity)}%`, sanity / 100, sanColor, pal);
    ly += 26;
    const dashCd = p.dashCooldown || 0;
    const dashPct = dashCd > 0 ? Math.max(0, 1 - dashCd / 0.8) : 1;
    drawMiniBar(leftX + innerPad, ly, innerW, 'THRUST', dashPct >= 1 ? 'READY' : `${dashCd.toFixed(1)}s`, dashPct, dashPct >= 1 ? pal.accent : pal.accentDim, pal);
    ly += 32;

    // Sub silhouette
    const subCx = leftX + railW / 2, subCy = ly + 18;
    drawHullDiagram(subCx, subCy, p, pal);

    // ----- RIGHT RAIL: COMBAT COMPUTER -----
    const rxL = rightX;
    drawRailFrame(rxL, topY, railW, railH, pal, 'COMBAT COMPUTER');

    const innerPadR = 14;
    const innerWR = railW - innerPadR * 2;
    const rcx = rxL + railW / 2;
    let ry = topY + 26;

    // ----- OBJECTIVES — clean stacked list (Hades-style quest panel) -----
    if (g.objectives && g.objectives.length) {
        ctx.textAlign = 'left'; ctx.font = 'bold 11px monospace';
        ctx.fillStyle = hexA(pal.accent, 0.7);
        ctx.fillText('DIVE BRIEF', rxL + innerPadR, ry + 4);
        ry += 12;
        for (const o of g.objectives) {
            const oh = 36;
            // Card
            ctx.fillStyle = o.complete ? 'rgba(40,80,40,0.55)' : 'rgba(8,16,24,0.85)';
            ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry, innerWR, oh, 4); ctx.fill();
            ctx.strokeStyle = o.complete ? '#80E0A0' : hexA(pal.accent, 0.4); ctx.lineWidth = 1;
            ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry, innerWR, oh, 4); ctx.stroke();
            // Status icon
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.font = 'bold 12px monospace'; ctx.textAlign = 'left';
            ctx.fillText(o.complete ? '✓' : '◯', rxL + innerPadR + 6, ry + 14);
            // Brief (wraps)
            ctx.fillStyle = o.complete ? '#A0E8B0' : pal.text;
            ctx.font = 'bold 11px monospace';
            const briefMaxW = innerWR - 24;
            const brief = o.brief.length * 5.5 > briefMaxW ? o.brief.slice(0, Math.floor(briefMaxW / 5.5) - 1) + '…' : o.brief;
            ctx.fillText(brief, rxL + innerPadR + 22, ry + 14);
            // Progress bar
            const pct = Math.max(0, Math.min(1, o.progress / o.target));
            ctx.fillStyle = 'rgba(0,0,0,0.6)';
            ctx.fillRect(rxL + innerPadR + 22, ry + 22, briefMaxW - 4, 4);
            ctx.fillStyle = o.complete ? '#80E0A0' : pal.accent;
            ctx.fillRect(rxL + innerPadR + 22, ry + 22, (briefMaxW - 4) * pct, 4);
            // Progress text
            ctx.fillStyle = pal.textDim; ctx.font = '11px monospace';
            const pTxt = (o.type === 'depth') ? `${Math.floor(o.progress)}/${o.target}m`
                       : (o.type === 'time' || o.type === 'no_hit') ? `${Math.floor(o.progress)}/${o.target}s`
                       : `${Math.floor(o.progress)}/${o.target}`;
            ctx.fillText(pTxt, rxL + innerPadR + 22, ry + 32);
            ry += oh + 4;
        }
        ry += 8;
    }

    // SIGNAL × YIELD — sonar-themed last-hit readout
    const sc = g.scoreCombo || {};
    const ev = sc.lastEvent;
    const recent = ev && (g.runTime - ev.time < 2.5);
    const sigW = (innerWR - 18) / 2;
    // SIGNAL
    ctx.fillStyle = '#0A1422';
    ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry, sigW, 32, 5); ctx.fill();
    ctx.strokeStyle = hexA('#80B0FF', 0.5); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry, sigW, 32, 5); ctx.stroke();
    ctx.fillStyle = '#3A5060'; ctx.font = 'bold 10px monospace'; ctx.textAlign = 'center';
    ctx.fillText('SIGNAL', rxL + innerPadR + sigW / 2, ry + 11);
    ctx.fillStyle = '#80B0FF'; ctx.font = 'bold 16px monospace';
    ctx.fillText(recent ? `${ev.chips}` : '—', rxL + innerPadR + sigW / 2, ry + 26);
    // ×
    ctx.fillStyle = pal.text; ctx.font = 'bold 14px monospace';
    ctx.fillText('×', rcx, ry + 21);
    // YIELD
    const yldX = rcx + 9;
    const yldW = sigW;
    ctx.fillStyle = '#1A0A0A';
    ctx.beginPath(); ctx.roundRect(yldX, ry, yldW, 32, 5); ctx.fill();
    const multHi = recent && ev.mult >= 3;
    ctx.strokeStyle = multHi ? '#FFD040' : hexA('#DA6040', 0.5); ctx.lineWidth = multHi ? 1.5 : 1;
    ctx.beginPath(); ctx.roundRect(yldX, ry, yldW, 32, 5); ctx.stroke();
    ctx.fillStyle = '#5A2A20'; ctx.font = 'bold 10px monospace';
    ctx.fillText('YIELD', yldX + yldW / 2, ry + 11);
    ctx.fillStyle = multHi ? '#FFD040' : '#DA6040'; ctx.font = 'bold 16px monospace';
    ctx.fillText(recent ? `${ev.mult.toFixed(1)}×` : '—', yldX + yldW / 2, ry + 26);
    ry += 40;

    // Active combo tags (one per row, full-width)
    if (recent && ev.tags.length) {
        ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        const showTags = ev.tags.slice(0, 4);
        for (let ti = 0; ti < showTags.length; ti++) {
            const tag = showTags[ti];
            ctx.fillStyle = 'rgba(255,200,80,0.18)';
            ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry + ti * 14, innerWR, 12, 3); ctx.fill();
            ctx.strokeStyle = 'rgba(255,200,80,0.35)'; ctx.lineWidth = 1;
            ctx.beginPath(); ctx.roundRect(rxL + innerPadR, ry + ti * 14, innerWR, 12, 3); ctx.stroke();
            ctx.fillStyle = '#FFD080';
            ctx.fillText(tag, rxL + innerPadR + 4, ry + ti * 14 + 9);
        }
        ry += showTags.length * 14 + 8;
    }

    // CHAIN — circular dial style
    const chain = sc.chainCount || 0;
    if (chain > 1) {
        const chR = 26;
        const chCy = ry + chR + 6;
        // Background ring
        ctx.strokeStyle = '#091420'; ctx.lineWidth = 4;
        ctx.beginPath(); ctx.arc(rcx, chCy, chR, 0, PI2); ctx.stroke();
        // Timer fill
        const ctPct = Math.min(1, (sc.chainTimer || 0) / 1.2);
        ctx.strokeStyle = chain >= 10 ? '#FFD040' : pal.accent; ctx.lineWidth = 4;
        ctx.beginPath(); ctx.arc(rcx, chCy, chR, -Math.PI / 2, -Math.PI / 2 + ctPct * PI2); ctx.stroke();
        // Number
        ctx.fillStyle = chain >= 10 ? '#FFD040' : pal.accent;
        ctx.font = 'bold 18px monospace'; ctx.textAlign = 'center';
        ctx.fillText(`${chain}`, rcx, chCy + 6);
        ctx.fillStyle = pal.textDim; ctx.font = 'bold 10px monospace';
        ctx.fillText('CHAIN', rcx, chCy + chR + 12);
        ry = chCy + chR + 22;
    }

    // Variety
    if (sc.uniqueTypes && sc.uniqueTypes.size > 0) {
        ctx.fillStyle = pal.textDim; ctx.font = 'bold 10px monospace'; ctx.textAlign = 'center';
        ctx.fillText('VARIETY', rcx, ry + 8);
        ctx.fillStyle = sc.uniqueTypes.size >= 3 ? '#80E0FF' : pal.text;
        ctx.font = 'bold 13px monospace';
        ctx.fillText(`${sc.uniqueTypes.size} types`, rcx, ry + 22);
        ry += 30;
    }

    // Threat — bottom of rail
    let nearby = 0, closestD = Infinity, closest = null;
    for (const e of g.enemies) {
        if (e.ghost) continue;
    ...
    ctx.fillStyle = pal.textDim; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
    ctx.fillText('THREAT', rcx, botY - 50);
    const threatColor = nearby > 30 ? '#DA4060' : nearby > 15 ? '#DAA520' : pal.accent;
    ctx.fillStyle = threatColor; ctx.font = 'bold 22px monospace';
    ctx.fillText(`${nearby}`, rcx, botY - 28);
    if (closest && closestD < 500) {
        ctx.fillStyle = pal.textDim; ctx.font = '11px monospace';
        ctx.fillText(`closest ${Math.floor(closestD)}m`, rcx, botY - 12);
    }
}
...
function drawObstacle(sx, sy, ob) {
    ...
    if (ob.kind === 'rock') {
        ...
        for (let i = 0; i <= 12; i++) {
            const a = (i / 12) * PI2;
            const rad = r * (0.85 + Math.sin(i * 1.7 + ob.seed) * 0.12);
        ...
        ctx.beginPath(); ctx.arc(-r * 0.3, -r * 0.3, r * 0.5, 0, PI2); ctx.fill();
    } else if (ob.kind === 'kelp') {
        // Tall swaying frond cluster
        const sway = Math.sin(performance.now() * 0.001 + ob.seed) * 0.15;
        ctx.strokeStyle = ob.color; ctx.lineWidth = 4; ctx.lineCap = 'round';
        for (let s = 0; s < 5; s++) {
            ctx.beginPath();
            const baseX = (s - 2) * 4;
            ctx.moveTo(baseX, r * 0.8);
            for (let seg = 1; seg <= 6; seg++) {
                const t = seg / 6;
                ctx.lineTo(baseX + sway * 25 * t, r * 0.8 - t * r * 1.6);
            }
            ctx.stroke();
        }
        // Base mound
        ctx.fillStyle = '#1A2820';
        ctx.beginPath(); ctx.ellipse(0, r * 0.7, r * 0.7, r * 0.3, 0, 0, PI2); ctx.fill();
        ...
        ctx.beginPath(); ctx.moveTo(-r * 0.55, r * 0.95); ctx.lineTo(0, -r * 1.15); ctx.stroke();
...
function drawHUD(w, h, g, pal, vpR) { /* DEAD CODE — replaced by drawMinimalHUD */ return;
    if (!pal) pal = getDepthPalette(g.depth || 0);
    const pp = g.player;
    const sanity = 100 - (pp.corruption || 0);
    const rimR = vpR + 4;
    const arcCy = 70 + (h - 130) / 2; // match viewport center
    ctx.save();

    // ===== VIEWPORT ARC BARS (quick glance — no labels, just colour) =====
    const arcW = 5;

    // HULL arc — left
    { const hpPct = Math.max(0.01, pp.hp / pp.maxHp);
      ctx.strokeStyle = '#0C1418'; ctx.lineWidth = arcW + 2;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, Math.PI*0.6, Math.PI*1.0); ctx.stroke();
      ctx.strokeStyle = hpPct > 0.5 ? '#30AA50' : hpPct > 0.25 ? '#CCAA30' : '#DD3030';
      ctx.lineWidth = arcW;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, Math.PI*0.6, Math.PI*0.6 + Math.PI*0.4*hpPct); ctx.stroke();
    }
    // XP arc — top
    { const xpPct = pp.xp / xpForLevel(pp.level);
      ctx.strokeStyle = '#0C1418'; ctx.lineWidth = arcW + 2;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, Math.PI*1.15, Math.PI*1.85); ctx.stroke();
      ctx.strokeStyle = pal.accent; ctx.lineWidth = arcW;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, Math.PI*1.15, Math.PI*1.15 + Math.PI*0.7*xpPct); ctx.stroke();
    }
    // SANITY arc — right
    { const sanPct = Math.max(0.01, sanity / 100);
      ctx.strokeStyle = '#0C1418'; ctx.lineWidth = arcW + 2;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, Math.PI*0.0, Math.PI*0.4); ctx.stroke();
      ctx.strokeStyle = sanPct > 0.6 ? '#4070CC' : sanPct > 0.3 ? '#9040AA' : '#CC2050';
      ctx.lineWidth = arcW;
      ctx.beginPath(); ctx.arc(w/2, arcCy, rimR, 0, Math.PI*0.4*sanPct); ctx.stroke();
    }

    // ===== COMBO + STREAK (anchored to viewport, not screen) =====
    const _vpCx = g._vpCx != null ? g._vpCx : w/2;
    const _vpCy = g._vpCy != null ? g._vpCy : h/2;
    if (g.combo > 5) {
        ctx.textAlign = 'center';
        ctx.fillStyle = 'rgba(0,0,0,0.4)'; ctx.font = 'bold 15px monospace';
        ctx.fillText(`x${g.combo}`, _vpCx+1, _vpCy + vpR - 20);
        ctx.fillStyle = g.combo > 50 ? '#FF6040' : g.combo > 25 ? '#FFD040' : pal.text;
        ctx.fillText(`x${g.combo}`, _vpCx, _vpCy + vpR - 21);
    }
    if (g.streakTimer > 0) {
        ctx.globalAlpha = Math.min(1, g.streakTimer * 0.8);
        ctx.textAlign = 'center';
        ctx.fillStyle = 'rgba(0,0,0,0.5)'; ctx.font = 'bold 20px monospace';
        ctx.fillText(g.streak, _vpCx+1, _vpCy - 45);
        ctx.fillStyle = pal.accent;
        ctx.fillText(g.streak, _vpCx, _vpCy - 47);
        ctx.globalAlpha = 1;
    }
    if (sanity < 40) {
        ctx.globalAlpha = 0.2 + Math.sin(g.runTime * 3) * 0.1;
        ctx.fillStyle = '#AA3060'; ctx.font = '11px monospace'; ctx.textAlign = 'center';
        ctx.fillText('NEURAL DEGRADATION', _vpCx, _vpCy + vpR - 8);
        ctx.globalAlpha = 1;
    }

    // ===== BOTTOM PANEL REMOVED — all info now lives in the side panels =====
    if (false) {
    const PANEL_H = 100;
    const panelY = h - PANEL_H;
    const panelGrad = ctx.createLinearGradient(0, panelY, 0, h);
    panelGrad.addColorStop(0, '#0A1620');
    panelGrad.addColorStop(1, '#040A12');
    ctx.fillStyle = panelGrad;
    ctx.fillRect(0, panelY, w, PANEL_H);
    ctx.strokeStyle = hexA(pal.accentDim, 0.45); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(0, panelY); ctx.lineTo(w, panelY); ctx.stroke();
    // Tiny scanlines
    ctx.fillStyle = 'rgba(127,168,150,0.025)';
    for (let yy = panelY + 4; yy < h; yy += 3) ctx.fillRect(0, yy, w, 1);

    // ----- LEFT: WEAPON BANK -----
    const wepLabels = { sonar: 'SN', torpedo: 'TP', field: 'EF', depthcharge: 'DC', harpoon: 'HP', lure: 'LR', tsunami: 'TS', leviathan_lance: 'LL', abyssal_mine: 'AM' };
    const wepStartX = 12;
    const slotW = 46, slotH = 78, gap2 = 4;
    ctx.font = 'bold 10px monospace'; ctx.textAlign = 'left';
    ctx.fillStyle = hexA(pal.accent, 0.55);
    ctx.fillText('ARMORY', wepStartX, panelY + 12);
    for (let i = 0; i < pp.weapons.length; i++) {
        const ww = pp.weapons[i];
        const def = WEAPON_DEFS[ww.id];
        const cdMax = def.baseCooldown * pp.cdMult;
        const cdPct = Math.max(0, Math.min(1, ww.cooldown / cdMax));
        const sx = wepStartX + i * (slotW + gap2);
        const sy = panelY + 16;
        const ready = cdPct <= 0;
        // Slot
        ctx.fillStyle = '#050D16';
        ctx.beginPath(); ctx.roundRect(sx, sy, slotW, slotH, 4); ctx.fill();
        ctx.strokeStyle = ready ? hexA(pal.accent, 0.7) : '#0C1B2A'; ctx.lineWidth = ready ? 1.4 : 1;
        ctx.beginPath(); ctx.roundRect(sx, sy, slotW, slotH, 4); ctx.stroke();
        // Cooldown fill (bottom-up)
        if (!ready) {
            const fillH = slotH * (1 - cdPct);
            ctx.fillStyle = hexA(pal.accent, 0.12);
            ctx.fillRect(sx + 1, sy + slotH - fillH, slotW - 2, fillH);
        } else {
            ctx.fillStyle = hexA(pal.accent, 0.10 + Math.sin(g.runTime * 4 + i) * 0.04);
            ctx.beginPath(); ctx.roundRect(sx, sy, slotW, slotH, 4); ctx.fill();
        }
        // Cooldown ring inside slot
        const rcX = sx + slotW / 2, rcY = sy + 22, rcR = 14;
        ctx.strokeStyle = '#091420'; ctx.lineWidth = 2.5;
        ctx.beginPath(); ctx.arc(rcX, rcY, rcR, 0, PI2); ctx.stroke();
        ctx.strokeStyle = ready ? pal.accent : hexA(pal.accent, 0.55);
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.arc(rcX, rcY, rcR, -Math.PI / 2, -Math.PI / 2 + (1 - cdPct) * PI2);
        ctx.stroke();
        // Weapon code
        ctx.fillStyle = ready ? '#A0DDD0' : '#3A5A50'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
        ctx.fillText(wepLabels[ww.id] || '??', rcX, rcY + 4);
        // Level pip row
        for (let pi = 0; pi < 8; pi++) {
            const filled = pi < ww.level;
            ctx.fillStyle = filled ? (def.evolved ? '#FFD040' : pal.accent) : '#1A3040';
            ctx.fillRect(sx + 4 + pi * ((slotW - 8) / 8), sy + slotH - 10, ((slotW - 8) / 8) - 1, 3);
        }
        // Name
        ctx.fillStyle = ready ? '#7FA896' : '#2B4A3A'; ctx.font = '9px monospace';
        ctx.fillText(def.name.toUpperCase().slice(0, 8), rcX, sy + slotH - 14);
        // Evolved star
        if (def.evolved) {
            ctx.fillStyle = '#FFD040'; ctx.font = 'bold 11px monospace';
            ctx.fillText('★', sx + slotW - 6, sy + 9);
        }
    }
    const wepEndX = wepStartX + pp.weapons.length * (slotW + gap2);

    // ----- RIGHT: ACTIVE PERKS / SYNERGIES + GOLD + DEFIANCE -----
    const rightX = w - 12;
    ctx.textAlign = 'right'; ctx.font = 'bold 10px monospace';
    ctx.fillStyle = hexA(pal.accent, 0.55);
    ctx.fillText('LOADOUT', rightX, panelY + 12);
    // Gold + defiance row
    ctx.font = 'bold 13px monospace';
    ctx.fillStyle = '#DAA520';
    ctx.fillText(`${g.goldEarned}g`, rightX, panelY + 30);
    if (pp.deathDefiance > 0) {
        ctx.fillStyle = '#FFD040'; ctx.font = 'bold 11px monospace';
        ctx.fillText(`◆ DEFIANCE ×${pp.deathDefiance}`, rightX, panelY + 46);
    }
    // Synergy chips (one per active synergy/card name)
    const syns = (g.activeSynergies || []).slice(-6);
    let cy0 = panelY + 60;
    let cx0 = rightX;
    ctx.font = 'bold 10px monospace';
    for (let si = syns.length - 1; si >= 0; si--) {
        const txt = syns[si];
        const cw = ctx.measureText(txt).width + 10;
        cx0 -= cw + 4;
        if (cx0 < w - 280) break; // don't overflow into center
        ctx.fillStyle = 'rgba(120,80,180,0.25)';
        ctx.beginPath(); ctx.roundRect(cx0, cy0, cw, 14, 3); ctx.fill();
        ctx.strokeStyle = 'rgba(180,120,255,0.55)'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(cx0, cy0, cw, 14, 3); ctx.stroke();
        ctx.fillStyle = '#E0BFFF'; ctx.textAlign = 'center';
        ctx.fillText(txt.toUpperCase(), cx0 + cw / 2, cy0 + 10);
    }
    // Active card count summary
    if (g.selectedCards && g.selectedCards.length) {
        ctx.textAlign = 'right';
        ctx.fillStyle = pal.textDim; ctx.font = '10px monospace';
        ctx.fillText(`${g.selectedCards.length} CARDS`, rightX, panelY + 90);
    }

    // ----- CENTER: pilot LEVEL + XP bar + ability indicators -----
    const cLeft = wepEndX + 16;
    const cRight = w - 240; // reserve room for synergy chips
    const cAvailable = Math.max(180, cRight - cLeft);
    const cW = Math.min(cAvailable, 480); // cap width for readability
    const cX = cLeft, cY = panelY + 10;
    // Level number — big
    ctx.textAlign = 'left';
    ctx.fillStyle = pal.textDim; ctx.font = 'bold 10px monospace';
    ctx.fillText('PILOT LEVEL', cX, cY + 8);
    ctx.fillStyle = '#A0DDD0'; ctx.font = 'bold 28px monospace';
    ctx.fillText(`${pp.level}`, cX, cY + 36);
    // XP bar
    const xpNeeded = xpForLevel(pp.level);
    const xpPct3 = Math.max(0, Math.min(1, pp.xp / xpNeeded));
    const xpX = cX + 60, xpY = cY + 24, xpW = cW - 70, xpH = 14;
    ctx.fillStyle = '#040A14';
    ctx.beginPath(); ctx.roundRect(xpX, xpY, xpW, xpH, 6); ctx.fill();
    ctx.strokeStyle = '#0C1B2A'; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.roundRect(xpX, xpY, xpW, xpH, 6); ctx.stroke();
    // Fill
    if (xpPct3 > 0) {
        const xpGrad = ctx.createLinearGradient(xpX, xpY, xpX + xpW, xpY);
        xpGrad.addColorStop(0, '#3A8A78');
        xpGrad.addColorStop(1, '#80E8C8');
        ctx.fillStyle = xpGrad;
        ctx.beginPath(); ctx.roundRect(xpX + 1, xpY + 1, (xpW - 2) * xpPct3, xpH - 2, 5); ctx.fill();
        // Shimmer
        ctx.fillStyle = 'rgba(255,255,255,0.08)';
        ctx.beginPath(); ctx.roundRect(xpX + 1, xpY + 1, (xpW - 2) * xpPct3, (xpH - 2) / 2, [5,5,0,0]); ctx.fill();
    }
    // XP value
    ctx.font = '11px monospace'; ctx.textAlign = 'right';
    ctx.fillStyle = pal.textDim;
    ctx.fillText(`${pp.xp} / ${xpNeeded} XP`, xpX + xpW, xpY - 2);
    // Label
    ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
    ctx.fillStyle = pal.text;
    ctx.fillText('XP', xpX, xpY - 2);
    // Ability indicators row (DASH, PING) — two compact pill buttons
    // Layout: [KEY] LABEL ........... STATE   (so labels never overlap state values)
    const abY = cY + 58;
    const pillW = 124, pillH = 22, pillGap = 8;
    function drawAbilityPill(x, y, key, label, state, ready, color) {
        ctx.fillStyle = ready ? hexA(color, 0.20) : 'rgba(20,30,40,0.55)';
        ctx.beginPath(); ctx.roundRect(x, y, pillW, pillH, 4); ctx.fill();
        ctx.strokeStyle = ready ? color : '#1A3040'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(x, y, pillW, pillH, 4); ctx.stroke();
        // Key cap
        ctx.fillStyle = ready ? hexA(color, 0.32) : 'rgba(40,60,80,0.5)';
        ctx.beginPath(); ctx.roundRect(x + 4, y + 3, 36, pillH - 6, 3); ctx.fill();
        ctx.fillStyle = ready ? '#FFF' : pal.textDim; ctx.font = 'bold 10px monospace'; ctx.textAlign = 'center';
        ctx.fillText(key, x + 22, y + pillH / 2 + 3);
        // Label
        ctx.fillStyle = ready ? '#DDEAE2' : pal.textDim; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'left';
        ctx.fillText(label, x + 44, y + pillH / 2 + 3);
        // State (right) — only render if there is room
        ctx.font = 'bold 11px monospace'; ctx.textAlign = 'right';
        ctx.fillStyle = ready ? color : pal.textDim;
        ctx.fillText(state, x + pillW - 6, y + pillH / 2 + 3);
    }
    const dashReady = pp.dashCooldown <= 0;
    drawAbilityPill(xpX, abY, 'SPACE', 'DASH', dashReady ? 'READY' : `${pp.dashCooldown.toFixed(1)}s`, dashReady, pal.accent);
    if (pp._sonarManual && !pp._sonarAuto) {
        const sw = pp.weapons.find(ww => ww.id === 'sonar');
        if (sw) {
            const pingReady = sw.cooldown <= 0;
            drawAbilityPill(xpX + pillW + pillGap, abY, 'F', 'PING', pingReady ? 'READY' : `${sw.cooldown.toFixed(1)}s`, pingReady, '#80FFE0');
        }
    }

    } // end of removed bottom panel block

    // --- SALVAGE PROMPT (when near a wreck) ---
    if (g.nearestWreck) {
        const wr = g.nearestWreck;
        const pulse = 0.6 + Math.sin(g.runTime * 4) * 0.3;
        const promptY = (g._vpCy || h / 2) + (g._vpR || 200) + 18;
        ctx.textAlign = 'center'; ctx.font = 'bold 11px monospace';
        ctx.fillStyle = `rgba(255, 208, 64, ${pulse})`;
        const prompt = wr.revealed ? `[ HOLD E ]  SALVAGE  →  ${wr.loot.label}` : '[ HOLD E ]  SALVAGE  →  ??? (ping to scan)';
...
// =====================================================================
function drawMinimap(w, h, g, pal) { /* DEAD CODE */ return;
    const mmR = 50;
    // Tucked into the right rail's bottom — clear of the porthole
    const railW = g._railW || 200;
    const mmCx = w - railW / 2;
    const mmCy = h - 110 - mmR - 18;
    const scale = mmR / 500; // 500px game = full minimap radius
    if (!pal) pal = getDepthPalette(g.depth || 0);

    ctx.save();
    // Clip to circle
    ctx.beginPath(); ctx.arc(mmCx, mmCy, mmR, 0, PI2); ctx.clip();
    ...
    for (let gx = -mmR; gx <= mmR; gx += 20) for (let gy = -mmR; gy <= mmR; gy += 20) {
        ctx.fillRect(mmCx + gx, mmCy + gy, 1, 1);
    }

    // Feature 7: Sonar sweep line
    const sweepA = (g.runTime || 0) * 0.8;
    ctx.strokeStyle = hexA(pal.accent, 0.5); ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(mmCx, mmCy);
    ctx.lineTo(mmCx + Math.cos(sweepA) * mmR, mmCy + Math.sin(sweepA) * mmR);
    ctx.stroke();

    // Sweep revealed sector (faint)
    ctx.fillStyle = hexA(pal.accent, 0.04);
    ctx.beginPath(); ctx.moveTo(mmCx, mmCy);
    ctx.arc(mmCx, mmCy, mmR, sweepA - 0.4, sweepA); ctx.closePath(); ctx.fill();
    ...
    for (const e of g.enemies) {
        if (e.ghost) continue;
        const ex = (e.x - p.x) * scale, ey = (e.y - p.y) * scale;
        const len = Math.sqrt(ex*ex+ey*ey);
        const mx = len > mmR ? (ex/len)*(mmR-2) : ex;
        const my = len > mmR ? (ey/len)*(mmR-2) : ey;
        const dotAlpha = len > mmR ? 0.35 : 0.8;
        ctx.globalAlpha = dotAlpha;
        ctx.fillStyle = e.isBoss ? '#FF4040' : '#FF6060';
        ctx.beginPath(); ctx.arc(mmCx + mx, mmCy + my, e.isBoss ? 3 : 1.5, 0, PI2); ctx.fill();
    }
    // Gems
    ctx.fillStyle = '#4AE0FF';
    for (const gem of g.gems) {
        const ex = (gem.x - p.x) * scale, ey = (gem.y - p.y) * scale;
        if (Math.sqrt(ex*ex+ey*ey) > mmR) continue;
        ctx.globalAlpha = 0.7;
        ctx.beginPath(); ctx.arc(mmCx + ex, mmCy + ey, 1, 0, PI2); ctx.fill();
    }
    // Depth charges + lures
    ctx.fillStyle = '#FFD040';
    for (const dc of [...g.depthCharges, ...g.lures]) {
        const ex = (dc.x - p.x) * scale, ey = (dc.y - p.y) * scale;
        if (Math.sqrt(ex*ex+ey*ey) > mmR) continue;
        ctx.globalAlpha = 0.9;
        ctx.beginPath(); ctx.arc(mmCx + ex, mmCy + ey, 2, 0, PI2); ctx.fill();
    }
    // Debris (white)
    ctx.fillStyle = 'rgba(150,170,180,0.4)';
    for (const d of g.debris) {
        const ex = (d.x - p.x) * scale, ey = (d.y - p.y) * scale;
        if (Math.sqrt(ex*ex+ey*ey) > mmR) continue;
        ctx.globalAlpha = 0.4;
        ctx.fillRect(mmCx + ex - 0.5, mmCy + ey - 0.5, 1, 1);
    }
    // Player dot (center, bright)
    ctx.globalAlpha = 1;
    ctx.fillStyle = '#FFF';
    ctx.beginPath(); ctx.arc(mmCx, mmCy, 2.5, 0, PI2); ctx.fill();

    ctx.restore();

    // Border
    ctx.strokeStyle = hexA(pal.accent, 0.5); ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.arc(mmCx, mmCy, mmR, 0, PI2); ctx.stroke();
    // Outer glow
    ctx.strokeStyle = hexA(pal.accent, 0.15); ctx.lineWidth = 3;
    ctx.beginPath(); ctx.arc(mmCx, mmCy, mmR + 1, 0, PI2); ctx.stroke();
}
...
function drawTouchControls(w, h) {
    ...
    // Dash button
    const dbx = w - 60, dby = panelY - 80;
    ctx.strokeStyle = '#FFD040'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(dbx, dby, 30, 0, PI2); ctx.stroke();
    ctx.fillStyle = touchDash.active ? 'rgba(255,210,40,0.5)' : 'rgba(255,210,40,0.2)';
    ctx.beginPath(); ctx.arc(dbx, dby, 30, 0, PI2); ctx.fill();
    ctx.fillStyle = '#FFD040'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
    ctx.fillText('DASH', dbx, dby + 4);
    ctx.globalAlpha = 1;
}
...
function drawTitle(w, h) {
    ...
    for (let i = 0; i < chars.length; i++) {
        ...
        if (unlocked) {
            ...
            ctx.fillText(ch.name, bx + cellW / 2, by + 72);
            // Tagline
            ctx.fillStyle = '#5A6A7A'; ctx.font = '10px monospace';
            ctx.fillText(ch.tagline || '', bx + cellW / 2, by + 86);
            // Mini stats row
            ctx.fillStyle = '#7A8A98'; ctx.font = '10px monospace';
            ctx.fillText(`HP ${ch.hp} · SPD ${ch.speed} · ${ch.crushDepth}m`, bx + cellW / 2, by + 100);
            // Key
    ...
    // Feature 8: Stake indicator — bottom row
    if (meta.highestStake > 0 || meta.currentStake > 0) {
        const stk = STAKES[meta.currentStake] || STAKES[0];
        ctx.textAlign = 'center';
        ctx.fillStyle = stk.color;
        ctx.font = 'bold 12px monospace';
        ctx.fillText(`STAKE: [${meta.currentStake}] ${stk.name.toUpperCase()} — ${stk.desc}`, w / 2, h - 56);
        ctx.fillStyle = '#556'; ctx.font = '10px monospace';
        ctx.fillText(`[S] Cycle stakes (0-${meta.highestStake} unlocked)`, w / 2, h - 38);
    }
...
function drawShop(w, h) {
    ...
    ctx.font = '14px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('[ESC] Back', w / 2, 110 + upgrades.length * 55 + 30);
}
...
function drawCodex(w, h) {
    ...
    // Feature 9: Creature scan entries
    ctx.fillStyle = '#5AAFDA'; ctx.font = 'bold 12px monospace';
    ctx.fillText('— CREATURE BESTIARY —', w / 2, 88);
    let ly = 108;
    ctx.font = '10px monospace'; ctx.textAlign = 'left';
    for (const [typeId, typeData] of Object.entries(ENEMY_TYPES)) {
        if (ly > 260) break;
        const scanned = meta.scannedCreatures.includes(typeId);
        const aberrantScanned = meta.aberrantScanned.includes('aberrant_' + typeId);
        ctx.fillStyle = scanned ? '#7FA896' : '#3A4A4A';
        // Unscanned: name hidden — show ??? + classification placeholder
        ctx.fillText(`${scanned ? '✓' : '○'} ${scanned ? typeData.name : '???'}`, 40, ly);
        if (scanned) {
            ctx.fillStyle = '#4A6A5A';
            ctx.fillText(`HP:${Math.floor(typeData.hp)} DMG:${typeData.damage} +10% scan bonus`, 160, ly);
        } else {
            ctx.fillStyle = '#2A3A3A';
            ctx.fillText('UNKNOWN CONTACT — scan with sonar to classify', 160, ly);
        }
        if (aberrantScanned) {
            ctx.fillStyle = '#9A4ABA';
            ctx.fillText('★ ABERRANT logged', w - 200, ly);
        }
        ly += 16;
    }

    // Zone scan completion
    ...
    ctx.fillText(`Bestiary: ${scannedCount}/${totalCreatures} (${pct}%) — +${Math.floor(scannedCount/totalCreatures*5)*5}% global dmg bonus`, w/2, 270);

    // Lore fragments
    ctx.strokeStyle = '#1A2A3A'; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(20, 282); ctx.lineTo(w-20, 282); ctx.stroke();
    ctx.fillStyle = '#5ADFCF'; ctx.font = 'bold 11px monospace'; ctx.textAlign = 'center';
    ctx.fillText('— LORE FRAGMENTS —', w/2, 298);
    ...
    for (const frag of LORE_FRAGMENTS) {
        ...
        if (unlocked) {
            ...
            ctx.fillText('[ FRAGMENT LOCKED ]', 40, ly);
            ly += 22;
        }
    }
...
window.addEventListener('keydown', e => {
    ...
    if (phase === 'title') {
        ...
        // Feature 8: Stake cycling
        if (e.key === 's' || e.key === 'S') {
            meta.currentStake = (meta.currentStake + 1) % (meta.highestStake + 1);
            saveMeta();
        }
        // Character select
        const num = parseInt(e.key);
    ...
    if (phase === 'shop') {
        if (e.key === 'Escape') phase = meta.totalRuns > 0 && game && game.player.hp <= 0 ? 'death' : 'title';
